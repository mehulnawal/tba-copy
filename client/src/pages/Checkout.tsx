...
export default function Checkout() {
  ...
  const applyReferenceId = async () => {
    try {
      const result = await checkoutApi.applyReferenceId(referenceId);
      ...
      setReferenceMessage("Invalid Reference ID");
    }
  };
  const removeReferenceId = async () => {
    const result = await checkoutApi.removeReferenceId();
    setSummary(result.summary);
  ...
  const place = async () => {
    ...
    try {
      ...
      new R({
        ...
        amount: d.razorpayOrder.amount,
        currency: d.razorpayOrder.currency,
        handler: async (response: unknown) => {
          try {
            ...
            queryClient.setQueryData(["cart"], (current: any) =>
              ...
              <div className="border border-gray-200 rounded-lg p-5 bg-white shadow-sm space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      disabled={!!appliedReferenceId}
                      className="flex-1 bg-white border border-gray-300 text-gray-900 text-xs px-3 py-2.5 rounded"
                    />
                    <button
                      type="button"
                      onClick={() => void applyReferenceId()}
                      disabled={!referenceId || !!appliedReferenceId}
                      className="bg-black text-white text-xs px-4 rounded disabled:opacity-50"
                    >
                      Apply
                    </button>
                  </div>
                  {appliedReferenceId && (
                    <div className="text-xs text-emerald-700">
                      {appliedReferenceId} applied{" "}
                      <button
                        type="button"
                        onClick={() => void removeReferenceId()}
                  ...
                  {referenceMessage && (
                    <p
                      className={
                        referenceMessage === "Invalid Reference ID"
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    >
                      {applyingCode === manualCouponCode.trim()
                        ? "Applying..."
                        : "Apply"}
                    </button>
                  </form>
                ...
                {appliedCoupons.map((coupon) => (
                  <div
                    key={coupon.code}
                    className="p-4 bg-emerald-50 border border-emerald-200 rounded flex items-center justify-between"
                  >
                    <div>
                    ...
                    <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
                      {availableCoupons.map((coupon) => {
                        ...
                        const isApplied = appliedCoupons.some(
                          (entry) => entry.code === coupon.code,
                        );
                        const isThisApplying = applyingCode === coupon.code;
                        return (
                          <div
                            key={coupon.code}
                            className={`p-3.5 border rounded-md transition-all flex items-start justify-between gap-3 ${isEligible ? "border-gray-200 bg-gray-50/50 hover:border-gray-400" : "border-gray-200 bg-gray-50/20 opacity-75"}`}
                          >
                            <div className="space-y-1 min-w-0 flex-1">
                              ...
                              {!isEligible && (
                                <div className="text-[11px] font-medium text-amber-700 pt-1">
                                  Add {formatCurrency(minReq - currentSubtotal)}{" "}
              ...
              {summary && (
                <div className="border border-gray-200 rounded-lg p-5 bg-white shadow-sm space-y-4">
                  ...
                  <div className="space-y-2.5 text-xs text-gray-600 tracking-wide">
                    ...
                    {summary.referenceDiscount ? (
                      <div className="flex justify-between text-emerald-700">
                        <span>Reference ID Discount</span>
                        <span className="font-medium">
                          -{formatCurrency(summary.referenceDiscount)}
                        </span>
                      </div>
                    ) : null}
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-gray-900">
                        {formatCurrency(summary.taxableSubtotal)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>GST (3%)</span>
