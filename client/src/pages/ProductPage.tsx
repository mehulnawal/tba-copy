...
const responsiveImage = (url: string, width = 800) =>
...
export default function ProductPage({
  ...
  const [selectedKaratFilter, setSelectedKaratFilter] = useState<
    "14kt" | "18kt"
  >((params.get("karat") as "14kt" | "18kt") || "14kt");
  ...
  const [pendingWishlistProduct, setPendingWishlistProduct] =
  ...
  const { data: products = [], isLoading: loading } = useQuery({
    ...
    queryFn: () => {
      const b2bParams = new URLSearchParams({ metal });
      ["minPrice", "maxPrice", "karat"].forEach((key) => {
        const value = params.get(key);
        if (value) b2bParams.set(key, value);
      });
      return apiRequest<Product[]>(
        b2b
          ? `/b2b/products?${b2bParams.toString()}`
          : `/products/${metal}?${query}`,
      );
    },
    staleTime: 2 * 60 * 1000,
    gcTime: 15 * 60 * 1000,
  ...
  const parentId = (category: Category) =>
    category.parent ? categoryId(category.parent) : null;
  const categoryOptions =
    metal === "gold"
      ? categories.filter(
          (category) =>
            category.categoryKind === "subcategory" &&
            parentId(category) ===
              categories.find((root) => root.categoryKind === "metal-root")
                ?._id,
        )
      : categories
          .filter((category) => category.categoryKind === "type")
          .flatMap((type) => [
            type,
            ...categories.filter(
              (child) => categoryId(child.parent) === type._id,
            ),
          ]);
  const selectCategory = (category: Category | null) => {
    ...
    } else if (category.categoryKind === "type") {
      next.set("mainCategory", category._id);
      next.delete("subCategory");
    } else if (category.parent) {
      next.set("mainCategory", parentId(category) || "");
      next.set("subCategory", category._id);
    } else {
      next.set("mainCategory", category._id);
      next.delete("subCategory");
    }
  ...
  const activeCategoryName =
    categories.find(
      (category) =>
        category._id === (selectedSubCategory || selectedMainCategory),
    )?.name || "All";
    ...
    try {
      if (alreadySaved) {
        ...
        const productPrices = Array.isArray(product.prices)
          ...
          : [];
        const priceObj =
          productPrices.find((price) => price.karat === "14kt") ||
          productPrices[0];
        await addToWishlistMutation.mutateAsync({
          productId: product.SKU,
          karat: priceObj?.karat || "14kt",
        });
  ...
  const filteredProducts = products.filter(
    (product) =>
      (!selectedMainCategory ||
        categoryId(product.mainCategory) === selectedMainCategory) &&
      (!selectedSubCategory ||
        categoryId(product.subCategory) === selectedSubCategory) &&
      (metal !== "gold" ||
        product.prices?.some((price) => price.karat === selectedKaratFilter)),
  );
  const clearFilters = () => {
    ...
    <div className="catalog-page min-h-screen flex flex-col antialiased">
      {!b2b && (
        <Navbar
          ...
          onCategoryChange={(cat) =>
            changeParam("category", cat === "All" ? null : cat)
          }
        />
      )}
                ...
                {PRICE_BUCKETS.map((bucket) => {
                  const isActive =
                    !bucket.min && !bucket.max
                      ...
                      : params.get("minPrice") === bucket.min &&
                        params.get("maxPrice") === bucket.max;
                  return (
                    <label
                      key={bucket.label}
                      className="flex items-center space-x-3 cursor-pointer text-sm text-gray-600 hover:text-gray-900"
                    >
                      <input
                        type="checkbox"
                        checked={isActive}
                        onChange={() =>
                          handlePriceBucketChange(bucket.min, bucket.max)
                        }
                        className="h-4 w-4 rounded border-gray-300 text-amber-600 focus:ring-amber-500"
                      />
                      <span
                        className={isActive ? "text-amber-700 font-medium" : ""}
                      >
                        {bucket.label}
                      </span>
                    </label>
                ...
                <div className="flex gap-2 pt-2">
                  {[
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setSelectedKaratFilter(option.value as "14kt" | "18kt");
                        changeParam("karat", option.value || null);
                      }}
                      className={`rounded border px-3 py-2 text-xs font-semibold ${selectedKaratFilter === option.value ? "border-amber-600 bg-amber-50 text-amber-800" : "border-gray-200 text-gray-600 hover:bg-gray-50"}`}
                    >
                      {option.label}
                    </button>
                  ))}
            ...
            {loading ? (
              ...
              <div className="text-center py-24 bg-white rounded-lg border border-dashed border-gray-300 p-8">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Try modifying search criteria or extending pricing limits.
                </p>
                <div className="mt-6">
                  <button
                    onClick={clearFilters}
      ...
      <div className="catalog-mobile-bar fixed inset-x-0 bottom-0 z-[var(--z-sticky)] flex gap-3 border-t border-[var(--color-border)] bg-[var(--color-bg-secondary)]/95 p-3 shadow-[var(--shadow-lg)] backdrop-blur lg:hidden">
        <button
          ...
          className="flex-1 rounded-[var(--radius-sm)] border border-[var(--color-teal)] px-4 py-3 text-sm font-semibold text-[var(--color-teal)]"
        >
          Filters
        </button>
        <button
          onClick={() => setIsSortMobileOpen(true)}
          className="flex-1 rounded-[var(--radius-sm)] bg-[var(--color-teal)] px-4 py-3 text-sm font-semibold text-white"
        >
          Sort options
        </button>
      </div>
      ...
      {isFilterMobileOpen && (
        <div
          className="catalog-sheet-backdrop fixed inset-0 z-[var(--z-modal)] flex items-end bg-black/40 lg:hidden"
          onClick={() => setIsFilterMobileOpen(false)}
        >
          <div
            className="catalog-bottom-sheet relative max-h-[85vh] w-full rounded-t-[var(--radius-xl)] bg-[var(--color-bg-secondary)] shadow-[var(--shadow-lg)] py-5 pb-10 flex flex-col overflow-y-auto px-5 animate-[slideUp_220ms_ease-out]"
            ...
            <div className="mt-3 space-y-4 flex-grow">
              <div>
                ...
                <button
                  ...
                  className={`block py-1 text-sm text-left w-full ${!selectedMainCategory ? "text-amber-700 font-bold" : "text-gray-600"}`}
                >
                  All Categories
                </button>
                {categoryOptions.map((c) => (
                  <button
                    ...
                    className={`block py-1 text-sm text-left w-full ${metal === "gold" ? "" : c.categoryKind === "type" ? "font-semibold text-gray-900" : "ml-4 border-l border-gray-200 pl-3"} ${selectedSubCategory === c._id || selectedMainCategory === c._id ? "text-amber-700 font-bold" : "text-gray-600"}`}
                  >
                    {c.name}
                  </button>
                ))}
              ...
              <div className="border-t pt-4">
                ...
                {PRICE_BUCKETS.map((b) => (
                  <button
                    key={b.label}
                    onClick={() => {
                      handlePriceBucketChange(b.min, b.max);
                      setIsFilterMobileOpen(false);
                    }}
                    className={`block py-1 text-sm text-left w-full ${(params.get("minPrice") || "") === b.min && (params.get("maxPrice") || "") === b.max ? "text-amber-700 font-bold" : "text-gray-600"}`}
                  >
                    {b.label}
                  </button>
                ))}
      ...
      {isSortMobileOpen && (
        <div
          className="catalog-sheet-backdrop fixed inset-0 z-[var(--z-modal)] flex items-end lg:hidden bg-black/40"
          onClick={() => setIsSortMobileOpen(false)}
        >
          <div
            className="catalog-bottom-sheet relative w-full bg-[var(--color-bg-secondary)] shadow-[var(--shadow-lg)] rounded-t-[var(--radius-xl)] p-6 space-y-4 animate-[slideUp_220ms_ease-out]"
            ...
            <div className="space-y-3">
              {SORT_OPTIONS.map((opt) => (
                <button
                  key={opt.value}
                  onClick={() => {
                    changeParam("sort", opt.value || null);
      ...
      <AuthModal
        ...
        onAuthenticated={handleWishlistAuthenticated}
      />
    </div>
  );
}
...
function FilterSection({
  ...
  return (
    <div className="border-b border-gray-200 py-4">
      ...
      {isOpen && (
        <div className="pt-2 transition-all duration-300">{children}</div>
      )}
    </div>
  );
}
...
export function ProductCard({
  ...
  defaultKarat: "14kt" | "18kt";
  onWishlistToggle: (product: Product) => void;
  isWishlisted: boolean;
  b2b?: boolean;
}) {
  const categoryName = (category?: Category | string | null) => {
  ...
  const targetPriceObj =
    productPrices.find((p) => p.karat === activeKarat) || productPrices[0];
  const displaysPrice = targetPriceObj
    ? Math.round(
        b2b
          ? (targetPriceObj.b2bFinalPrice ?? targetPriceObj.finalPrice)
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/5] w-full bg-gray-50 overflow-hidden">
        ...
        >
          {images[currentImgIndex] ? (
            <img
              src={responsiveImage(images[currentImgIndex], 800)}
              alt={product.title}
              className="h-full w-full object-contain object-center transition duration-500 group-hover:scale-102"
            />
          ) : (
        ...
        {displaysCarousel && (
          <>
            <button
              ...
              className="absolute left-3 top-1/2 z-10 -translate-y-1/2 rounded-full bg-white/90 p-2 text-gray-700 shadow"
            ...
            <button
              ...
              className="absolute right-3 top-1/2 z-10 -translate-y-1/2 rounded-full bg-white/90 p-2 text-gray-700 shadow"
      ...
      <div className="flex flex-1 flex-col p-4 space-y-2">
        <span className="text-xs sm:text-[10px] tracking-wider text-gray-400 uppercase font-semibold">
          {categoryLabel}
        </span>
        ...
        >
          <h3
            className="product-title normal-case text-lg leading-snug text-gray-900 hover:text-amber-700 line-clamp-2 min-h-[48px] transition sm:text-xl"
          ...
          <div className="pt-3 mt-auto border-t border-gray-100 flex items-center justify-between">
            ...
            <div className="flex space-x-1">
              {(["14kt", "18kt"] as const).map((kt) => {
                const hasPurityVariant = (product.prices ?? []).some(
                  (p) => p.karat === kt,
                );
                return (
                  <button
