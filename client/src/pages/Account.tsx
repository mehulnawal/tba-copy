...
export default function Account() {
  ...
  const [activeTab, setActiveTab] = useState<"profile" | "addresses">(
    searchParams.get("tab") === "addresses" ? "addresses" : "profile",
  );
  useEffect(() => {
    setActiveTab(
      searchParams.get("tab") === "addresses" ? "addresses" : "profile",
  ...
  const handleAddressSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const { fullName, phone, houseNo, area, city, state, pincode, country } =
      addressForm;
    if (
      !fullName ||
      !phone ||
        ...
        <div className="container max-w-5xl mx-auto">
          ...
          <div className="flex flex-wrap gap-8 mb-10 border-b border-[var(--color-border-subtle)] pb-2">
            <button
              ...
              className={`font-secondary pb-2 text-sm uppercase tracking-widest transition-all duration-300 relative ${
                activeTab === "profile"
                    ...
                    >
                      <div>
                        ...
                        <p className="font-secondary text-xs text-[var(--color-text)] space-y-1">
                          ...
                          <span className="block text-[var(--color-text-muted)] mt-2">
                            Ph: {address.phone}
                          </span>
                        </p>
                      </div>
