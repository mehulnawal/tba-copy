...
export default function CartPage() {
  ...
  const cart: CartItem[] = useMemo(
    () =>
      (cartData?.items || []).map((item) => {
        ...
        const hasCategoryCoupon = Boolean(
          item.categoryCouponApplied && item.categoryCouponDiscount,
        );
        return {
          lineSubtotal,
  ...
  const [pincodeStatus, setPincodeStatus] = useState<
    "idle" | "checking" | "valid" | "invalid" | "unsupported"
  >("idle");
            ...
            {isPageLoading ? (
              ...
              >
                <div className="lg:col-span-8 space-y-6">
                  {[1, 2].map((i) => (
                    <div
                      key={i}
                      className="flex gap-4 sm:gap-6 border-b border-[var(--color-border-subtle)] pb-6"
                    >
                    ...
                    <AnimatePresence mode="popLayout">
                      {cart.map((item) => (
                        <motion.div
                          ...
                          layout
                          className={`grid grid-cols-1 sm:grid-cols-12 gap-4 sm:gap-2 py-6 items-center px-2 transition-opacity duration-300 ${
                            false ? "opacity-65" : "opacity-100"
                          }`}
                        >
                          ...
                          <div className="col-span-1 sm:col-span-6 flex gap-4 sm:gap-6 items-center">
                            ...
                            <div className="space-y-1 min-w-0">
                              ...
                              <div className="font-display text-xs text-[var(--color-text-muted)] pt-0.5">
                                Piece Value:{" "}
                                {item.hasCategoryCoupon ? (
                                  <>
                                    <span className="line-through">
                                      {formatINR(item.originalPieceValue)}
                                    </span>
                                    <span className="ml-2 font-medium text-[var(--color-text)]">
                                      {formatINR(item.discountedPieceValue)}
                                    </span>
                                  </>
                                ) : (
                                  formatINR(item.discountedPieceValue)
                              ...
                              {item.hasCategoryCoupon &&
                                item.categoryCouponLabel && (
                                  <div className="font-secondary text-[10px] tracking-wide text-[var(--color-teal)] pt-0.5">
                                    Category Coupon:{" "}
                                    {item.categoryCouponLabel.replace(
                                      /[^0-9A-Z% .]+/g,
                              ...
                              <div className="flex sm:hidden items-center gap-4 pt-3">
                                <div className="flex items-center border border-[var(--color-border-subtle)] bg-[var(--color-bg-secondary)]">
                                  ...
                                  <span className="w-6 text-center font-display text-xs">
                                    {item.quantity}
                                  </span>
                                  <button
                                    type="button"
                          ...
                          <div className="hidden sm:col-span-3 sm:flex flex-col items-center justify-center gap-2">
                            <div className="flex items-center border border-[var(--color-border-subtle)] bg-[var(--color-bg-secondary)] shadow-xs">
                              ...
                              <span className="w-8 text-center font-display text-xs select-none">
                                {item.quantity}
                              </span>
                              <button
                                type="button"
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-[var(--color-border-subtle)] text-xs">
                                |
                              </span>
                              <button
                                onClick={() => handleRemoveItem(item.id)}
              ...
              >
                ...
                <p className="font-secondary text-xs sm:text-sm text-[var(--color-text-muted)] tracking-wide font-light max-w-xs mx-auto leading-relaxed mb-8">
                  The collection portfolio is currently unassigned. Explore our
                  fine rings, curated necklaces, and bespoke seasonal releases.
                </p>
                <button
