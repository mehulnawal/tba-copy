...
const TESTIMONIALS: Testimonial[] = [
  ...
  },
  {
    ...
    rating: 5,
    review:
      "Found Solitaire Ring IGI Certified at the most reasonable price in the market. Genuine and loved the finishing",
    images: [
      "https://res.cloudinary.com/dkrchgmhx/image/upload/v1786531509/WhatsApp_Image_2026-08-06_at_4.37.36_PM_wmlbms.jpg",
      "https://res.cloudinary.com/dkrchgmhx/image/upload/v1786538802/WhatsApp_Image_2026-08-06_at_4.37.36_PM-2_kznihk.jpg",
    ],
  },
];
...
const FAQS: FAQ[] = [
  ...
  },
  {
    ...
    answer:
      "There is absolutely no visual or physical way to differentiate between a lab diamond and a natural diamond by looking at them. Trained jewellers or experts cannot tell them apart using naked eyes or regular microscopes. Identifying a lab-grown diamond requires specialized, advanced spectroscopic equipment found inside certified gemological laboratories.",
  },
  {
    id: "4",
    question:
      "Are Lab-Grown Diamonds worth buying and do you provide buy back?",
    answer:
      "Absolutely. Lab-grown diamonds are worth buying because they are LIKE real diamonds that last forever. TBA offers direct to customer pricing, because of which it is not reasonable to provide buy back. Exchange can be done at any point of time.",
  },
  {
    id: "5",
    question: "Do Lab-Grown Diamonds cost less than mined diamonds?",
    answer:
      "Yes, they do. Lab-grown diamonds generally retail at a significant 60% to 90% discount compared to mined diamonds of equivalent quality. Despite the accessible price point, they are evaluated, graded, and priced using the exact same standard matrix Cut, Color, Clarity, and Carat weight.",
  },
  {
    id: "6",
    question: "Are Lab-Grown Diamonds graded and certified?",
    answer:
      "Yes. Lab-grown diamonds undergo the same rigorous grading procedures as mined diamonds. They are certified by leading global gemological laboratories, such as IGI, SGL, and GCI. Every single lab-grown diamond piece used by TBA is carefully graded and certified by these renowned, third-party organizations.",
  },
  {
    id: "7",
    question:
      "Are Lab-Grown Diamonds the same as Synthetic Diamonds or Moissanite?",
    answer:
      "No, lab-grown diamonds are not diamond simulants like Cubic Zirconia (CZ) or Moissanite, which only imitate the look of a diamond. Lab-grown diamonds are pure carbon crystals and constitute genuine diamonds, completely distinct from synthetic imitations.",
  },
  {
    id: "8",
    question: "Are Lab-Grown Diamonds durable enough to last?",
    answer:
      "Diamonds are the hardest known material on Earth, ranking at a maximum 10 on the MOHs Scale of hardness. Lab-grown diamonds share this exact same rank of 10, delivering identical structural strength, hardness, and ultimate durability. You can confidently pass these timeless pieces down to the next generation.",
  },
  {
    id: "9",
    question:
      "Do Lab-Grown Diamonds change colour, shine, or sparkle over time?",
    answer:
      "No, they do not. Lab-grown diamonds will never fade, discolor, lose their brilliance, or turn cloudy over time. Their optical sparkle and natural fire are permanent and timeless.",
  },
  {
    id: "10",
    question: "Do all Lab-Grown Diamonds look alike?",
    answer:
      "No. While the growth environment is controlled in modern machines, the diamond crystallization process is entirely organic and natural. Just like mined diamonds, nature dictates the outcome, resulting in unique variances in Cut, Colour, Clarity, and Carat size for every individual piece.",
  },
];
...
export default function HomePage() {
  ...
  useEffect(() => {
    Promise.all([
      apiRequest<ApiProduct[]>(
        `/products/gold?search=${encodeURIComponent(searchQuery)}`,
      ),
      apiRequest<ApiProduct[]>(
        `/products/silver?search=${encodeURIComponent(searchQuery)}`,
      ),
    ])
      .then(([gold, silver]) => {
        ...
        setProducts(
          activeCategory === "All"
            ? merged
            : merged.filter((product) => product.Category === activeCategory),
        );
      })
      .catch((err) =>
        console.error("Error loading home catalogue products:", err),
      );
  ...
  useEffect(() => {
    if (sessionStorage.getItem("tba_homepage_popup_shown")) return;
    const timer = window.setTimeout(() => {
      setIsHomepagePopupOpen(true);
      sessionStorage.setItem("tba_homepage_popup_shown", "true");
    }, 5500);
  ...
  const bestSellerProducts = useMemo(() => {
    return products
      .filter((p) => p.Is_Best_Seller === true || p.isBestSeller === true)
  ...
  const homepageCategories = useMemo(
    () =>
      categories.filter((category) =>
        Boolean(
          category.showOnHomepage && category.categoryKind !== "metal-root",
        ),
      ),
    [categories],
  );
  const openCategory = (category: Category) => {
    ...
    const parentId =
      typeof category.parent === "string"
        ? category.parent
        : category.parent?._id || "";
    if (category.categoryKind === "subcategory") {
      params.set("mainCategory", parentId);
      params.set("subCategory", category._id);
    } else {
  ...
  const resolveLiveProductPrice = (product: Product): number => {
    ...
    if (!matchedRaw || !matchedRaw.prices || matchedRaw.prices.length === 0)
      return 0;
    // Lookup standard presentation variant or default first available index
    const targetPrice =
      matchedRaw.prices.find((p) => p.karat === product.karat) ||
      matchedRaw.prices[0];
  ...
  const addToCart = async (product: Product) => {
    ...
    try {
      await addToCartMutation.mutateAsync(
        productToCartPayload(product, activePrice),
      );
  ...
  const toggleWishlist = async (product: Product) => {
    ...
    const activePrice = resolveLiveProductPrice(product);
    const isInWishlist = apiWishlist?.some(
    ...
    try {
      if (isInWishlist) {
        ...
        await addToWishlistMutation.mutateAsync(
          productToWishlistPayload(product, activePrice),
        );
    ...
    <div className="min-h-screen bg-[var(--color-bg)] flex flex-col relative antialiased selection:bg-[var(--color-cream)] selection:text-[var(--color-teal)]">
      ...
      <main className="flex-1 w-full flex flex-col gap-0">
            ...
            >
              ...
              <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex items-center gap-2.5 z-10">
                {heroSlides.map((_, i) => (
                  ...
                  >
                    <span
                      className={`block h-0.5 rounded-xs transition-all duration-500 ${
                        i === activeSlide
                          ? "w-10 bg-white"
                          : "w-3.5 bg-white/35"
                      }`}
        ...
        {homepageCategories.length > 0 && (
          <section
            className="py-10 md:py-14 bg-[var(--color-bg)]"
                  ...
                  return (
                    ...
                    >
                      {category.homepageCoverImage && (
                        <img
                          src={category.homepageCoverImage}
                          alt={category.name}
        ...
        <div className="container flex justify-center py-6 md:py-8">
          <button
            ...
            className="bg-[var(--color-teal)] px-7 py-3 font-secondary text-xs font-semibold uppercase tracking-widest text-[var(--color-cream)] transition hover:opacity-90"
          >
            Explore Collection
          </button>
        </div>
        ...
        {bestSellerProducts.length > 0 && (
          <section
            className="reveal-section py-3 bg-[var(--color-bg)]"
            id="collection-grid"
          >
            <div className="container">
              <div className="flex flex-col items-center mb-5 text-center">
                <span className="section-label">
                  Selected Statement Masterpieces
                </span>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <div className="flex gap-1.5">
                    {Array.from({
                      length: Math.floor(TESTIMONIALS[testimonialIndex].rating),
                    }).map((_, i) => (
                      <Star
                        key={i}
                        size={16}
                      ...
                      <div className="flex flex-wrap gap-2 sm:justify-end">
                        {testimonialImages.map((image, index) => (
                          <img
                            key={image}
                            src={image}
                            alt={`${testimonial.name}'s shared image ${index + 1}`}
                            className="h-16 w-16 rounded-md border border-[var(--color-border-subtle)] object-cover sm:h-20 sm:w-20"
                            loading="lazy"
                          />
                        ))}
                      </div>
        ...
        <section
          className="section-padding bg-[var(--color-bg-secondary)] border-t border-[var(--color-border-subtle)]"
          id="faq-section"
        >
          <div className="container max-w-4xl flex flex-col gap-10">
            ...
            <div className="w-full flex flex-col gap-3 reveal-section">
              {FAQS.slice(0, areAllFaqsVisible ? FAQS.length : 2).map((faq) => {
                ...
                return (
                  <div
                    key={faq.id}
                    className="bg-[var(--color-white)] rounded-lg border border-[var(--color-border)] overflow-hidden shadow-xs"
                  >
                    <button
            ...
            {FAQS.length > 2 && (
              <div className="flex justify-center reveal-section">
                <button
                  type="button"
                  onClick={() =>
                    setAreAllFaqsVisible((visible) => {
                      if (visible)
                  ...
                  className="w-full max-w-xs border border-[var(--color-teal)] bg-transparent px-6 py-3 text-xs font-semibold uppercase tracking-widest text-[var(--color-teal)] transition hover:bg-[var(--color-teal)] hover:text-white sm:w-auto"
                >
            ...
            >
              <div className="p-6 border-b border-[var(--color-border-subtle)] flex justify-between items-center">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="font-primary font-bold text-xl text-[var(--color-teal)]">
                    My Atelier Wishlist
                  </span>
                  <span className="bg-red-50 text-red-500 text-xs font-mono font-bold px-2 py-0.5 rounded-sm">
                    {apiWishlist?.length || 0} item
                    {(apiWishlist?.length || 0) !== 1 && "s"}
                  </span>
                </div>
                <button
                  onClick={() => setIsWishlistOpen(false)}
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-6 flex flex-col gap-4">
                {(apiWishlist?.length || 0) === 0 ? (
                  <div className="my-auto flex flex-col items-center gap-4 text-center py-10">
                    <Heart
                      size={48}
                      className="text-[var(--color-border)] stroke-[1.25px]"
                    />
                    <p className="font-primary italic text-lg text-[var(--color-teal)]">
                      Your wishlist is currently tranquil
                    </p>
                  </div>
                ) : (
                  (apiWishlist || []).map((item) => {
                    ...
                    return (
                      <div
                        key={item._id}
                        className="flex gap-4 border border-[var(--color-border-subtle)] p-4 rounded-lg bg-[var(--color-bg-secondary)] relative group"
                      >
                        ...
                        <div className="flex-1 flex flex-col justify-between py-0.5">
                          <div>
                            <h5 className="font-primary text-sm font-semibold text-[var(--color-teal)]">
                              {item.name}
                            </h5>
      ...
      <AnimatePresence>
        {isAuthOpen && (
          <AuthModal
            ...
            onAuthenticated={() => {
              if (authReturnTo) navigate(authReturnTo);
            }}
          />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {isHomepagePopupOpen && (
          <motion.div
            className="fixed inset-0 z-[var(--z-overlay)] flex items-center justify-center bg-black/55 p-4"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
            <motion.section
              role="dialog"
              aria-modal="true"
              aria-label="Homepage offer"
              className="relative w-full max-w-2xl overflow-hidden rounded-[var(--radius-md)] border border-[#cbb58b] bg-[linear-gradient(135deg,_#eee8dc,_#d9cfbd)] p-6 text-center shadow-[0_24px_70px_rgba(31,38,36,0.34)] sm:p-9"
            ...
            >
              <button
                type="button"
                onClick={() => setIsHomepagePopupOpen(false)}
                aria-label="Close popup"
                className="absolute right-4 top-4 z-10 rounded-full border border-[#173b3a]/15 bg-[#f7f2e9]/80 p-2 text-[#173b3a] transition hover:bg-white"
              >
                <X size={18} />
              </button>
              <h2 className="font-primary text-3xl text-[#173b3a]">
                Pay for the Jewellery, Not the Middleman
              </h2>
              <video
                className="mt-5 block max-h-[60vh] w-full rounded-[var(--radius-sm)] border border-[#173b3a]/15 bg-black object-contain shadow-[0_12px_30px_rgba(23,59,58,0.22)]"
              ...
              <button
                type="button"
                onClick={() => setIsHomepagePopupOpen(false)}
                className="mt-6 border border-[#173b3a] bg-[#173b3a] px-6 py-3 text-xs font-semibold uppercase tracking-widest text-white shadow-sm transition hover:bg-[#285653]"
              >
                Shop Now
              </button>
            </motion.section>
