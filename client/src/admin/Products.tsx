...
type FormField = "title" | "SKU" | "metal" | "mainCategory" | "subCategory" | "images" | "totalNumberOfDiamonds" | "diamonds" | "price" | "moissaniteCaratWeight";
...
function SilverPriceBreakup({ isMoissanite, grossWeight, caratWeight, adjustment, rates, onGrossWeightChange, onCaratWeightChange, onAdjustmentChange, caratWeightError }: { isMoissanite: boolean; grossWeight: number; caratWeight: number; adjustment: number; caratWeightError?: string; rates: { silver: number; makingRate: number; moissanite: number }; onGrossWeightChange: (value: number) => void; onCaratWeightChange: (value: number) => void; onAdjustmentChange: (value: number) => void }) {
...
export default function Products() {
  ...
  return <form onSubmit={(e) => void save(e, true)} onFocusCapture={(event) => { const input = event.target as HTMLInputElement; if (input.type === "number" && !input.readOnly && !input.disabled && input.value === "0") input.select(); }} onWheelCapture={(event) => { const input = event.target as HTMLInputElement; if (input.type === "number") { event.preventDefault(); input.blur(); } }} className="admin-editor"><header className="admin-page-heading"><div><p>Catalogue management</p><h1>{editing ? "Edit product" : "Create product"}</h1><span>Fields are aligned with the Product schema and current pricing engines.</span></div><button type="button" className="admin-secondary" onClick={() => setCreating(false)}>Back to products</button></header>{Object.keys(errors).length > 0 && <div className="admin-error-summary" role="alert">Please complete {Object.keys(errors).length} required field{Object.keys(errors).length === 1 ? "" : "s"}.</div>}
    ...
    {form.metal === "gold" ? <GoldWeightBlock karat="18kt" grossWeight={form.goldGrossWeight["18kt"]} diamondWeight={diamondWeightGrams} onGrossWeightChange={(value) => set("goldGrossWeight", { ...form.goldGrossWeight, "18kt": value })} /> : form.metal === "silver" ? <SilverPriceBreakup isMoissanite={isMoissanite} grossWeight={form.grossWeight} caratWeight={silverCaratWeight} adjustment={silverAdjustment} rates={silverRates} onGrossWeightChange={(value) => set("grossWeight", value)} onCaratWeightChange={(value) => { set("moissaniteEntries", [{ caratWeight: value }]); set("moissaniteCaratWeight", value); }} onAdjustmentChange={setSilverAdjustment} caratWeightError={errors.moissaniteCaratWeight} /> : null}
