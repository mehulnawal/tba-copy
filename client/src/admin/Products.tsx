...
type FormField = "title" | "SKU" | "metal" | "mainCategory" | "subCategory" | "images" | "totalNumberOfDiamonds" | "diamonds" | "price";
...
function SilverPriceBreakup({ isMoissanite, grossWeight, caratWeight, adjustment, rates, onGrossWeightChange, onCaratWeightChange, onAdjustmentChange }: { isMoissanite: boolean; grossWeight: number; caratWeight: number; adjustment: number; rates: { silver: number; makingRate: number; moissanite: number }; onGrossWeightChange: (value: number) => void; onCaratWeightChange: (value: number) => void; onAdjustmentChange: (value: number) => void }) {
  ...
  return <section className="admin-section">
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <Field label="Gross Weight (g) - Manual entry" required hint="Used in Metal Price and Making Charge."><input className="admin-input" type="number" min="0" step="0.01" value={grossWeight} onChange={(e) => onGrossWeightChange(number(e.target.value))} /></Field>
      ...
      {isMoissanite && <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <Field label="Moissanite Carat (ct) - Manual entry" hint="Enter the total Moissanite carat weight."><input className="admin-input" type="number" min="0" step="0.01" value={caratWeight} onChange={(e) => onCaratWeightChange(number(e.target.value))} /></Field>
...
export default function Products() {
  ...
  const [creating, setCreating] = useState(false), [saving, setSaving] = useState(false), [uploading, setUploading] = useState<number | null>(null), [errors, setErrors] = useState<FormErrors>({}), [submitted, setSubmitted] = useState(false), [newDiamondCategory, setNewDiamondCategory] = useState(""), [newDiamondSubType, setNewDiamondSubType] = useState(""), [newDiamondClarity, setNewDiamondClarity] = useState("");
  const firstInvalidRef = useRef<HTMLElement | null>(null);
  const load = useCallback(async () => { const [products, categoryList, diamondClarityList, certificateList] = await Promise.all([adminApi.adminProducts(), adminApi.categories(), adminApi.diamondClarities(), adminApi.certificates()]); let diamondCategoryList: DiamondCategory[] = []; try { diamondCategoryList = await adminApi.diamondCategories(); } catch { diamondCategoryList = []; } setItems(products); setCategories(categoryList); setDiamondCategories((Array.isArray(diamondCategoryList) ? diamondCategoryList : []).map((row) => { const raw = row as DiamondCategory & { name?: string; category?: string; category_name?: string; categorySize?: string }; return { ...row, categoryName: raw.categoryName || raw.name || raw.category || raw.category_name || "", size: raw.size || raw.categorySize || "" }; })); setDiamondClarities(diamondClarityList); setCertificateOptions(certificateList); }, []);
  ...
  return <form onSubmit={(e) => void save(e, true)} className="admin-editor"><header className="admin-page-heading"><div><p>Catalogue management</p><h1>{editing ? "Edit product" : "Create product"}</h1><span>Fields are aligned with the Product schema and current pricing engines.</span></div><button type="button" className="admin-secondary" onClick={() => setCreating(false)}>Back to products</button></header>{Object.keys(errors).length > 0 && <div className="admin-error-summary" role="alert">Please complete {Object.keys(errors).length} required field{Object.keys(errors).length === 1 ? "" : "s"}.</div>}
    ...
    {form.metal === "gold" ? <GoldWeightBlock karat="18kt" grossWeight={form.goldGrossWeight["18kt"]} diamondWeight={diamondWeightGrams} onGrossWeightChange={(value) => set("goldGrossWeight", { ...form.goldGrossWeight, "18kt": value })} /> : form.metal === "silver" ? <SilverPriceBreakup isMoissanite={isMoissanite} grossWeight={form.grossWeight} caratWeight={silverCaratWeight} adjustment={silverAdjustment} rates={silverRates} onGrossWeightChange={(value) => set("grossWeight", value)} onCaratWeightChange={(value) => { set("moissaniteEntries", [{ caratWeight: value }]); set("moissaniteCaratWeight", value); }} onAdjustmentChange={setSilverAdjustment} /> : null}
    {form.metal && <section className="admin-section"><div className="admin-section-heading"><span>03</span><div><h2>Diamonds</h2><p>Choose a saved colour/clarity value for each diamond entry.</p></div></div><div className="admin-grid"><Field name="totalNumberOfDiamonds" label="Total Number of Diamonds" required error={errors.totalNumberOfDiamonds}><input {...fieldProps("totalNumberOfDiamonds")} className="admin-input" type="number" min="0" step="1" value={form.totalNumberOfDiamonds ?? ""} onChange={(e) => set("totalNumberOfDiamonds", e.target.value === "" ? undefined : Number(e.target.value))} /></Field></div>{errors.diamonds && <p className="admin-inline-error" role="alert">{errors.diamonds}</p>}{form.diamonds.map((diamond, index) => <div className="admin-grid" key={index}><Field label="Category"><select className="admin-input" value={diamond.category} onChange={(e) => { const matches = diamondCategories.filter((item) => item.categoryName === e.target.value); const row = matches.length === 1 ? matches[0] : undefined; updateDiamond(index, { category: e.target.value, diamondCategoryRef: row?._id, ratePerCtB2B: row?.b2bPrice ?? diamond.ratePerCtB2B, ratePerCtB2C: row?.b2cPrice ?? diamond.ratePerCtB2C }); }}><option value="">Select category</option>{diamondCategoryNames.map((name) => <option key={name} value={name}>{name}</option>)}</select></Field>{diamond.category && <Field label="Size"><select className="admin-input" disabled={diamondCategories.filter((item) => item.categoryName === diamond.category).length === 1} value={diamond.diamondCategoryRef || ""} onChange={(e) => { const row = diamondCategories.find((item) => item._id === e.target.value); updateDiamond(index, { diamondCategoryRef: row?._id, ratePerCtB2B: row?.b2bPrice ?? diamond.ratePerCtB2B, ratePerCtB2C: row?.b2cPrice ?? diamond.ratePerCtB2C }); }}><option value="">Select size</option>{diamondCategories.filter((item) => item.categoryName === diamond.category).map((item) => <option key={item._id} value={item._id}>{item.size}</option>)}</select></Field>}<Field label="Sub-type"><select className="admin-input" value={diamond.subType} onChange={(e) => updateDiamond(index, { subType: e.target.value })}><option value="">Select sub-type</option>{STONE_SUB_TYPES.map((subType) => <option key={subType} value={subType}>{subType}</option>)}</select></Field><Field label="Colour / clarity"><select className="admin-input" value={diamond.colorClarity} onChange={(e) => updateDiamond(index, { colorClarity: e.target.value })}><option value="">Select clarity</option>{diamondClarities.map((clarity) => <option key={clarity._id} value={clarity.name}>{clarity.name}</option>)}</select></Field><Field label="Carat weight"><input className="admin-input" type="number" min="0" step="0.01" value={diamond.caratWeight} onChange={(e) => updateDiamond(index, { caratWeight: number(e.target.value) })} /></Field><Field label="Rate per Ct (B2B)" required><input className="admin-input" type="number" min="0" value={diamond.ratePerCtB2B ?? ""} onChange={(e) => updateDiamond(index, { ratePerCtB2B: e.target.value === "" ? undefined : Number(e.target.value) })} /></Field><Field label="Rate per Ct (B2C)" required><input className="admin-input" type="number" min="0" value={diamond.ratePerCtB2C ?? ""} onChange={(e) => updateDiamond(index, { ratePerCtB2C: e.target.value === "" ? undefined : Number(e.target.value) })} /></Field><div className="admin-field"><span>Diamond Value (Calculated)</span><b>{currency(Number(diamond.caratWeight || 0) * Number(diamond.ratePerCtB2C ?? diamond.ratePerCt ?? 0))}</b></div><button type="button" className="admin-text-button danger" onClick={() => set("diamonds", form.diamonds.filter((_, i) => i !== index))}>Remove diamond</button></div>)}<div className="admin-karat-card"><b>Diamond totals</b><small>Total Diamond Weight: {diamondTotals.weight} ct</small><strong>Total Diamond Value: {currency(diamondTotals.value)}</strong></div><div className="admin-grid"><Field label="New diamond category"><input className="admin-input" value={newDiamondCategory} onChange={(event) => setNewDiamondCategory(event.target.value)} placeholder="e.g. Accent" /></Field><Field label="New sub-type"><input className="admin-input" value={newDiamondSubType} onChange={(event) => setNewDiamondSubType(event.target.value)} placeholder="e.g. Oval" /></Field><button type="button" className="admin-secondary" onClick={() => void saveDiamondOption()}>Save diamond option</button><Field label="New diamond clarity"><input className="admin-input" value={newDiamondClarity} onChange={(event) => setNewDiamondClarity(event.target.value)} placeholder="e.g. VS1" /></Field><button type="button" className="admin-secondary" onClick={() => void saveDiamondClarityOption()}>Save clarity option</button></div><button type="button" className="admin-secondary" onClick={addDiamond}>+ Add diamond</button></section>
    }
    <>{form.metal === "gold" && <section className="admin-section"><div className="admin-section-heading"><span>05</span><div><h2>Certificate Charge</h2><p>Manual weight replaces the legacy total-diamond-weight basis for gold certificate charges.</p></div></div><div className="admin-grid"><Field label="Universal Price"><input className="admin-input" readOnly value={currency(universalCertificatePrice)} /></Field><Field label="Weight"><input className="admin-input" required type="number" min="0" step="0.01" value={form.certificateWeight} onChange={(e) => set("certificateWeight", e.target.value)} /></Field><Field label="Certificate Charge (Universal Price × Weight)"><input className="admin-input" readOnly value={currency(universalCertificatePrice * Number(form.certificateWeight || 0))} /></Field></div></section>}</><section className="admin-section"><div className="admin-section-heading"><span>04</span><div><h2>Certificates</h2></div></div><div className="admin-grid"><Field label="Applicable certificates" hint="Certificate options are managed in the server code array."><CertificateMultiSelect options={certificateOptions} selectedIds={form.certificates} onChange={(certificates) => set("certificates", certificates)} /></Field></div></section>{form.metal === "gold" && <><GoldWeightBlock karat="14kt" grossWeight={form.goldGrossWeight["14kt"]} diamondWeight={diamondWeightGrams} onGrossWeightChange={(value) => set("goldGrossWeight", { ...form.goldGrossWeight, "14kt": value })} /><section className="admin-section"><div className="admin-section-heading"><span>05</span><div><h2>Weight & price preview</h2><p>Prices continue to use the connected Universal Gold and making rates.</p></div></div><GoldPricePreview preview={preview} /></section></>}
