...
import { colorForSlot, normalizeVariantImages, shortColor, variantConfig, YELLOW } from "../utils/productVariants";

type Karat = "14kt" | "18kt";
type Image = { url: string; source: "link" | "upload"; slot?: number };
type FormField = "title" | "SKU" | "metal" | "mainCategory" | "subCategory" | "images" | "colors" | "totalNumberOfDiamonds" | "diamonds" | "price" | "moissaniteCaratWeight" | "grossWeight";
...
const CAD_FORM_BASE_URL = import.meta.env.VITE_CAD_GOOGLE_FORM_BASE_URL || "https://docs.google.com/forms/d/e/1FAIpQLScQLwRnGi8gV7fw_WubYMC2TUrCKDx2hCZUv_0uGeL6MXr67Q/viewform?usp=pp_url&entry.1415814127=";

type CadSectionProps = { sku?: string; cadFolderUrl?: string; onRefresh: () => void; refreshing: boolean; showError: (message: string) => void; };
const CadSection = ({ sku, cadFolderUrl, onRefresh, refreshing, showError }: CadSectionProps) => {
  ...
  return <section className="admin-section">
    ...
    <div className="space-y-4">
      ...
      {isMoissanite && <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        {calculated("Moissanite Price", moissanitePrice, "Moissanite Carat x Moissanite Rate = Moissanite Price")}
      </div>}
...
export default function Products() {
  ...
  const validateProductForm = useCallback((state: ProductForm): FormErrors => {
    ...
    const variantProduct = { metal: state.metal as "gold" | "silver", mainCategory: categories.find((category) => category._id === state.mainCategory) }; const config = variantConfig(variantProduct); const images = normalizeVariantImages(state.images); if (!state.colors.length) next.colors = "Select at least one allowed color."; else if (state.colors.some((color) => !config.colors.includes(color))) next.colors = "Selected color is not allowed for this category."; else { const unselectedColor = images.map((image) => colorForSlot(variantProduct, image.slot || 0)).find((color) => color && !state.colors.includes(color)); if (unselectedColor) next.images = `Select ${shortColor(unselectedColor)} color or remove its images.`; } if (!images.length) next.images = "Add at least one product image.";
  ...
  const isPolkiSelection = form.metal === "silver" && /polki/i.test(categories.find((category) => category._id === form.mainCategory)?.name || "");
  useEffect(() => { if (isPolkiSelection && (form.colors.length !== 1 || form.colors[0] !== YELLOW)) set("colors", [YELLOW]); }, [isPolkiSelection, form.colors]);
  ...
  const updateImage = (index: number, image?: Image) => setForm((p) => ({ ...p, images: [...p.images.filter((entry, entryIndex) => (Number(entry.slot) || entryIndex + 1) !== index + 1), ...(image ? [{ ...image, slot: index + 1 }] : [])].sort((left, right) => Number(left.slot) - Number(right.slot)) }));
  const uploadImage = async (index: number, file: File) => { try { setUploading(index); const result = await adminApi.uploadImage(file); updateImage(index, { url: result.url, source: "upload", slot: index + 1 }); } catch { showToast("Image upload failed. Please try again.", "error"); } finally { setUploading(null); } };
  ...
  const refreshCadStatus = useCallback(async () => {
    ...
    try {
      ...
      setEditing((current) => current && (current.id || current._id) === productId ? { ...current, ...latest } : current);
    } catch {
      showToast("Could not refresh CAD status. Please try again.", "error");
    } finally { setRefreshingCad(false); }
  }, [editing, showToast]);
  useEffect(() => {
    if (!editing) return;
    const handleReturn = () => { if (document.visibilityState === "visible") void refreshCadStatus(); };
    window.addEventListener("focus", handleReturn);
    document.addEventListener("visibilitychange", handleReturn);
    return () => { window.removeEventListener("focus", handleReturn); document.removeEventListener("visibilitychange", handleReturn); };
  ...
  const openEdit = (product: Product) => { const metal = product.metal || "gold"; setEditing(product); setForm({ ...blank(), ...product, metal, mainCategory: categoryId(product.mainCategory), subCategory: categoryId(product.subCategory), images: normalizeVariantImages(product.images), sizes: STANDARD_RING_SIZES, colors: product.colors?.length ? product.colors : variantConfig({ metal, mainCategory: product.mainCategory }).colors, grossWeight: typeof product.grossWeight === "number" ? product.grossWeight : 0, netWeight: typeof product.netWeight === "number" ? product.netWeight : 0, goldGrossWeight: goldWeights(product.grossWeight), moissaniteEntries: product.moissaniteEntries || (product.moissaniteCaratWeight !== undefined ? [{ caratWeight: product.moissaniteCaratWeight }] : []), diamonds: (product.diamonds || []).map((diamond) => ({ ...diamond, ratePerCtB2B: diamond.ratePerCtB2B ?? diamond.ratePerCt, ratePerCtB2C: diamond.ratePerCtB2C ?? diamond.ratePerCt })), certificateWeight: String(product.certificateWeight ?? ""), certificates: (product.certificates || []).map((certificate) => typeof certificate === "string" ? certificate : certificate._id) }); setErrors({}); setSubmitted(false); setCreating(true); };
  const save = async (e: FormEvent, publish: boolean) => {
    ...
    try {
      ...
      const savedProduct = editing ? await adminApi.updateProduct(editing.id || editing._id || "", data) : await adminApi.createProduct(data);
      showToast(`Product ${editing ? "updated" : "created"} successfully.`, "success");
      openEdit(savedProduct); setErrors({}); setSubmitted(false); await load();
  ...
  const isPolki = form.metal === "silver" && /polki/i.test(selectedSilverType || "");
  const variantProduct = { metal: form.metal as "gold" | "silver", mainCategory: categories.find((category) => category._id === form.mainCategory) };
  const variantGroups = variantConfig(variantProduct).groups.map((slots, index) => ({ slots, color: variantConfig(variantProduct).colors[index] }));
  const imageAtSlot = (slot: number) => normalizeVariantImages(form.images).find((image) => image.slot === slot);
  ...
  return <form onSubmit={(e) => void save(e, true)} onFocusCapture={(event) => { const input = event.target as HTMLInputElement; if (input.type === "number" && !input.readOnly && !input.disabled && input.value === "0") input.select(); }} onWheelCapture={(event) => { const input = event.target as HTMLInputElement; if (input.type === "number") { event.preventDefault(); input.blur(); } }} className="admin-editor"><header className="admin-page-heading"><div><p>Catalogue management</p><h1>{editing ? "Edit product" : "Create product"}</h1><span>Fields are aligned with the Product schema and current pricing engines.</span></div><button type="button" className="admin-secondary" onClick={() => setCreating(false)}>Back to products</button></header>{Object.keys(errors).length > 0 && <div className="admin-error-summary" role="alert">Please complete {Object.keys(errors).length} required field{Object.keys(errors).length === 1 ? "" : "s"}.</div>}
    ...
    <CadSection sku={editing?.SKU || form.SKU} cadFolderUrl={editing?.cadFolderUrl} onRefresh={() => void refreshCadStatus()} refreshing={refreshingCad} showError={(message) => showToast(message, "error")} />
    ...
    <>{form.metal === "gold" && <section className="admin-section"><div className="admin-section-heading"><span>05</span><div><h2>Certificate Charge</h2><p>Manual weight replaces the legacy total-diamond-weight basis for gold certificate charges.</p></div></div><div className="admin-grid"><Field label="Universal Price"><input className="admin-input" readOnly value={currency(universalCertificatePrice)} /></Field><Field label="Weight"><input className="admin-input" required type="number" min="0" step="0.01" value={form.certificateWeight} onChange={(e) => set("certificateWeight", e.target.value)} /></Field><Field label="Certificate Charge (Universal Price x Weight)"><input className="admin-input" readOnly value={currency(universalCertificatePrice * Number(form.certificateWeight || 0))} /></Field></div></section>}</><section className="admin-section"><div className="admin-section-heading"><span>04</span><div><h2>Certificates</h2></div></div><div className="admin-grid"><Field label="Applicable certificates" hint="Certificate options are managed in the server code array."><CertificateMultiSelect options={certificateOptions} selectedIds={form.certificates} onChange={(certificates) => set("certificates", certificates)} /></Field></div></section>{form.metal === "gold" && <><GoldWeightBlock karat="14kt" grossWeight={form.goldGrossWeight["14kt"]} diamondWeight={diamondWeightGrams} grossWeightError={errors.grossWeight} onGrossWeightChange={(value) => set("goldGrossWeight", { ...form.goldGrossWeight, "14kt": value })} /><section className="admin-section"><div className="admin-section-heading"><span>05</span><div><h2>Weight & price preview</h2><p>Prices continue to use the connected Universal Gold and making rates.</p></div></div><GoldPricePreview preview={preview} /></section></>}
    <section className="admin-section"><div className="admin-section-heading"><span>{form.metal === "gold" ? "06" : "04"}</span><div><h2>Media & visibility</h2><p>Use the assigned colour slots. Sizes are standardized customer-side from 5 to 25.</p></div></div><>{errors.images && <p className="admin-inline-error" id="images-error" role="alert">{errors.images}</p>}{errors.colors && <p className="admin-inline-error" id="colors-error" role="alert">{errors.colors}</p>}{variantGroups.map(({ slots, color }) => <div key={color} className="mt-4"><b className="text-sm text-[var(--color-teal)]">{shortColor(color)} Images — Slots {slots[0]}–{slots[1]}</b><div className="admin-media-grid mt-2" data-field="images" tabIndex={-1} aria-invalid={Boolean(errors.images)} aria-describedby={errors.images ? "images-error" : undefined}>{slots.map((slot) => { const index = slot - 1; return <MediaCard key={slot} index={index} image={imageAtSlot(slot)} uploading={uploading === index} onUpload={(file) => void uploadImage(index, file)} onUrl={(url) => updateImage(index, url ? { url, source: "link", slot } : undefined)} onRemove={() => updateImage(index, undefined)} />; })}</div></div>)}</><Field label="External video URL"><input className="admin-input" type="url" value={form.videoLink} onChange={(e) => set("videoLink", e.target.value)} /></Field><div className="admin-colours" data-field="colors" tabIndex={-1}><b>Available metal colours <span className="text-red-600" aria-hidden="true">*</span></b><small className="block text-[var(--color-text-muted)]">Select at least one colour before saving.</small><div>{isPolki ? <label className="admin-colour-option selected"><input type="checkbox" checked readOnly disabled />{shortColor(YELLOW)}</label> : variantGroups.map(({ color }) => <label key={color} className={`admin-colour-option ${form.colors.includes(color) ? "selected" : ""}`}><input type="checkbox" checked={form.colors.includes(color)} onChange={(e) => set("colors", e.target.checked ? [...form.colors, color] : form.colors.filter((item) => item !== color))} />{shortColor(color)}</label>)}</div></div><div className="admin-toggle-grid"><Toggle label="Product visibility" checked={form.isActive} onChange={(value) => set("isActive", value)} /><Toggle label="Best seller" checked={form.isBestSeller} onChange={(value) => set("isBestSeller", value)} /><Toggle label="New arrival" checked={form.isNewProduct} onChange={(value) => set("isNewProduct", value)} /><Toggle label="Show in Prime Collection" checked={form.isPrimeCollection} onChange={(value) => set("isPrimeCollection", value)} /></div></section>
