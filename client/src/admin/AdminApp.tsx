...
import {
  ...
  type Coupon,
  type CategoryCoupon,
  type CategoryCouponCategory,
  type ManagedUser,
  type B2BAccessStatus,
  type PricingConfig,
} from "../api/admin.api";
...
function Toast({
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 z-[var(--z-float)] flex items-center gap-3 px-5 py-3.5 rounded-[var(--radius-md)] border shadow-[var(--shadow-lg)] text-sm font-secondary transition-all duration-300 ${badgeClasses}`}
    >
      <span>{message}</span>
      <button
        onClick={onClose}
        aria-label="Dismiss notification"
        className="opacity-70 hover:opacity-100 transition-opacity ml-2 font-bold cursor-pointer"
      >
        <X size={16} />
      </button>
    </div>
...
function Badge({
  ...
  const styles = {
    ...
    neutral:
      "bg-[var(--color-border)]/40 text-[var(--color-text-muted)] border-[var(--color-border)]",
    gold: "bg-[var(--color-cream)] text-[var(--color-teal-dark)] border-[var(--color-cream)]",
  };
  return (
    <span
      className={`inline-flex items-center px-3 py-1 rounded-[var(--radius-full)] text-xs font-semibold uppercase tracking-wider border ${styles[variant]}`}
    >
      {children}
    </span>
  );
...
function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center p-12 text-center border border-dashed border-[var(--color-border)] rounded-[var(--radius-lg)] bg-[var(--color-bg-secondary)]">
      <div className="text-4xl mb-3 text-[var(--color-teal-light)]">
        {icon || <Sparkles aria-hidden="true" />}
      </div>
      <h3 className="text-lg font-primary text-[var(--color-teal)]">{title}</h3>
      <p className="text-xs text-[var(--color-text-muted)] max-w-sm mt-1 font-secondary">
        {description}
      </p>
    </div>
  );
...
function LoadingState({ title = "Loading content..." }: { title?: string }) {
  return (
    <div
      className="p-6 border border-[var(--color-border)] rounded-[var(--radius-lg)] bg-[var(--color-bg-secondary)] space-y-4"
      aria-busy="true"
      aria-label={title}
    >
      ...
      <p className="text-xs font-secondary tracking-widest text-[var(--color-text-muted)] uppercase">
        {title}
      </p>
    </div>
  );
    ...
    <main className="min-h-screen grid place-items-center bg-[var(--color-bg)] p-6 font-secondary">
      <div className="w-full max-w-md bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-[var(--radius-lg)] p-8 shadow-[var(--shadow-lg)] space-y-6">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-primary text-[var(--color-teal)]">
            Portal Authentication
          </h1>
          <div className="teal-divider mx-auto my-2"></div>
        </div>
        {error && (
          <div className="p-3 bg-[var(--color-error)]/10 border border-[var(--color-error)]/30 text-[var(--color-error)] text-xs rounded-[var(--radius-md)]">
            {error}
          </div>
        )}
        <form onSubmit={submit} className="space-y-4">
          ...
          <button
            disabled={loading}
            className="admin-button w-full cursor-pointer disabled:opacity-50"
          >
...
function Layout({
  ...
  const navItems = [
    ...
    {
      to: "/admin/b2b-access-log",
      label: "B2B Access Log",
      icon: ClipboardList,
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg)] text-[var(--color-text)] flex flex-col md:flex-row font-secondary">
      <header className="md:hidden flex items-center justify-between p-4 border-b border-[var(--color-border)] bg-[var(--color-bg-secondary)]">
        <span className="font-primary font-bold text-[var(--color-teal)] tracking-wider">
          HAUTE jewellery
        </span>
        <button
          onClick={() => setMobileOpen(!mobileOpen)}
          className="p-2 text-[var(--color-teal)] font-bold"
        >
      ...
      >
        <div>
          <div className="p-6 border-b border-[var(--color-border)] hidden md:block">
            ...
            <h2 className="text-xl font-primary text-[var(--color-teal)] mt-1">
              Maison Control
            </h2>
          </div>
          ...
          <nav className="p-4 space-y-1">
            {navItems.map((item) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `admin-nav flex items-center gap-3 transition-all ${isActive ? "active" : ""}`
                }
              >
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      setStats({
        orders:
          o.status === "fulfilled"
            ? o.value?.orders?.length || o.value?.length || 0
            : 0,
  ...
  const cards = [
    {
      ...
      value: stats.orders,
      icon: ShoppingBag,
      desc: "Recent completed & pending purchases",
    },
    {
    ...
    },
    {
      ...
      value: stats.coupons,
      icon: Ticket,
      desc: "Live discounts and campaigns",
    },
  ];
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        subtitle="High-level platform activity and content status"
      />
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {cards.map((card, i) => (
          <div
            key={i}
            className="p-6 rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-secondary)] shadow-[var(--shadow-sm)] space-y-2"
          >
            ...
            <p className="text-4xl font-primary text-[var(--color-teal)]">
              {card.value}
            </p>
...
function Coupons() {
  ...
  useEffect(() => {
    if (edit) {
      ...
      setDiscountValue(
        edit.discountValue !== undefined ? String(edit.discountValue) : "",
      );
      setMinimumCartValue(
        edit.minimumCartValue !== undefined
          ? String(edit.minimumCartValue)
          : "0",
      );
      setExpiryDate(
        edit.expiryDate
          ? new Date(edit.expiryDate).toISOString().slice(0, 16)
          : "",
      );
      setUsageLimit(
        edit.usageLimit !== null && edit.usageLimit !== undefined
          ? String(edit.usageLimit)
          : "",
      );
  ...
  const save = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const parsedUsageLimit =
      usageLimit.trim() !== "" ? Number(usageLimit) : null;

    if (!trimmedCode)
      return setToast({ message: "Coupon code is required.", type: "error" });
    ...
    if (
      ...
      (isNaN(parsedUsageLimit) || parsedUsageLimit < 1)
    )
      return setToast({
        message: "Usage limit must be at least 1 or left blank.",
        type: "error",
      });
    if (!expiryDate)
      return setToast({ message: "Expiry date is required.", type: "error" });

    const expDateObj = new Date(expiryDate);
    if (isNaN(expDateObj.getTime()))
      return setToast({ message: "Invalid expiry date.", type: "error" });
  ...
  return (
    <div className="space-y-8">
      ...
      <PageHeader
        ...
        subtitle="Create and manage checkout promotional codes"
      />
      ...
      <form onSubmit={save} className="admin-form">
        ...
        <div className="col-span-full flex items-center justify-between pt-2">
          ...
          <div className="flex gap-2">
            ...
            <button
              disabled={isSaving}
              className="admin-button cursor-pointer disabled:opacity-50"
            >
              {isSaving
                ? "Saving..."
                : edit
      ...
      <div className="space-y-4">
        ...
        {filteredItems.length === 0 ? (
          <EmptyState
            ...
            description="Try creating a new coupon code or adjust your search filter."
            icon={<Sparkles aria-hidden="true" />}
          />
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredItems.map((c) => {
              ...
              return (
                <div
                  key={c._id}
                  className="admin-row flex flex-col justify-between"
                >
                  <div className="flex items-start justify-between w-full">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-bold text-lg text-[var(--color-teal)]">
                          {c.code}
                        </span>
                  ...
                  <div className="text-xs space-y-1 text-[var(--color-text-muted)] border-t border-[var(--color-border)] pt-3 w-full">
                    <p>
                      ...
                      <span className="font-semibold text-[var(--color-charcoal)]">
                        {formatCurrency(c.minimumCartValue)}
                      </span>
                    </p>
                    <p>
                      Usage:{" "}
...
function CategoryWiseCoupons() {
  ...
  const load = useCallback(async () => {
    setIsLoading(true);
    try {
      setItems(await adminApi.categoryCoupons());
    } catch (e) {
  ...
  const begin = (category: CategoryCouponCategory) => {
    ...
    setMinimumCartValue(coupon ? String(coupon.minimumCartValue) : "0");
    setExpiryDate(
      coupon?.expiryDate
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const value = Number(discountValue),
      minimum = Number(minimumCartValue),
      usage = usageLimit.trim() ? Number(usageLimit) : null;
    if (!code.trim())
      return setToast({ message: "Coupon code is required.", type: "error" });
    if (
    ...
    if (usage !== null && (!Number.isInteger(usage) || usage < 1))
      return setToast({
        message: "Usage limit must be at least 1 or left blank.",
        type: "error",
      });
  ...
  return (
    <section className="space-y-4 pt-2">
      ...
      <div>
        <h2 className="text-xl font-primary text-[var(--color-charcoal)]">
          Category-wise Coupons
        </h2>
        <p className="text-sm text-[var(--color-text-muted)] mt-1">
          Configure one independent coupon for each pricing category.
        </p>
      </div>
      {isLoading ? (
              ...
              <div key={key} className="admin-row flex flex-col gap-3">
                ...
                {coupon ? (
                  <div className="text-xs space-y-1 text-[var(--color-text-muted)]">
                    <p className="font-mono font-bold text-[var(--color-teal)]">
                      {coupon.code}
                    </p>
                    <p>
          ...
          <div className="col-span-full flex items-center justify-between pt-2">
            ...
            <div className="flex gap-2">
              <button
                type="button"
                onClick={reset}
                className="px-4 py-2 border border-[var(--color-border)] text-xs rounded-[var(--radius-sm)] cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                className="admin-button cursor-pointer disabled:opacity-50"
              >
                {isSaving ? "Saving..." : "Save Category Coupon"}
              </button>
            </div>
          </div>
        </form>
...
function Banners() {
  ...
  const save = async (p: BannerPayload) => {
    try {
      editing
        ? await adminApi.updateBanner(editing._id, p)
        : await adminApi.createBanner(p);
      setEditing(null);
  ...
  return (
    <div className="space-y-8">
      ...
      <PageHeader
        ...
        subtitle="Manage storefront promotional hero slots"
      />
      ...
      {items.length === 0 ? (
        <EmptyState
          ...
          description="Upload banner image files to showcase on your homepage."
          icon={<Sparkles aria-hidden="true" />}
        />
      ) : (
          ...
          {items.map((b) => (
            <div key={b._id} className="admin-row flex flex-col space-y-3">
              <img
                ...
                decoding="async"
                alt={`Banner ${b.order}`}
                className="w-full h-44 object-cover rounded-[var(--radius-md)] border border-[var(--color-border)]"
              />
              <div className="flex items-center justify-between text-xs w-full">
                <span className="text-[var(--color-text-muted)] font-semibold uppercase tracking-wider">
                  Slot Order:{" "}
                  <b className="text-[var(--color-teal)]">{b.order}</b>
                </span>
              ...
              <div className="flex items-center justify-end gap-2 border-t border-[var(--color-border)] pt-3 w-full">
                <button onClick={() => shift(b, -1)} className="cursor-pointer">
                  Move up Order
                </button>
                <button onClick={() => shift(b, 1)} className="cursor-pointer">
                  Move down Order
                ...
                <button
                  onClick={() =>
                    adminApi.setBannerActive(b._id, !b.isActive).then(load)
                  }
                  className="cursor-pointer"
                >
                ...
                <button
                  onClick={() =>
                    ...
                    adminApi.deleteBanner(b._id).then(load)
                  }
                  className="text-red-700 cursor-pointer"
                >
                  Delete
...
function BannerForm({
  ...
  return (
    ...
    >
      <b>
        {value
          ? `Editing Banner Slot ${value.order}`
...
function Announcements() {
  ...
  const save = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      edit
        ? await adminApi.updateAnnouncement(edit._id, p)
        : await adminApi.createAnnouncement(p);
      setEdit(null);
  ...
  return (
    <div className="space-y-8">
      ...
      <PageHeader
        ...
        subtitle="Manage header announcement bars and alerts"
      />

      <form
        key={edit ? edit._id : "new"}
        onSubmit={save}
        className="admin-form max-w-xl"
      >
        ...
        <div className="col-span-full flex justify-end gap-2">
          ...
          <button className="admin-button cursor-pointer">
            Save Announcement
          </button>
        </div>
      </form>
      ...
      {items.length === 0 ? (
        <EmptyState
          ...
          description="Create notice bar messages for your customers."
          icon={<Sparkles aria-hidden="true" />}
        />
      ) : (
        <div className="space-y-3">
          {items.map((a) => (
            <div key={a._id} className="admin-row justify-between">
              <div>
                <p className="text-sm text-[var(--color-charcoal)] font-semibold">
                  {a.message}
                </p>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() =>
                    adminApi
                      .setAnnouncementActive(a._id, !a.isActive)
                      .then(load)
                  }
                  className="cursor-pointer"
                >
                ...
                <button onClick={() => setEdit(a)} className="cursor-pointer">
                  Edit
                </button>
                <button
                  onClick={() =>
                    confirm("Delete announcement?") &&
                    adminApi.deleteAnnouncement(a._id).then(load)
                  }
                  className="text-red-700 cursor-pointer"
                >
                  Delete
...
function B2BAccessManagement() {
  ...
  const save = async (event: FormEvent) => {
    event.preventDefault();
    try {
      const next = await adminApi.setB2BPassword(password);
      setStatus(next);
      setToast({
        message:
          "B2B password activated. Existing B2B sessions are now invalid.",
        type: "success",
      });
    } catch (error) {
      setToast({ message: errorMessage(error), type: "error" });
  ...
  const revoke = async () => {
    if (
      !confirm(
        "Revoke B2B access now? This invalidates every active B2B session.",
      )
    )
      return;
    try {
      const next = await adminApi.revokeB2BPassword();
      setStatus(next);
      setPassword("");
      setToast({
        message: "B2B access revoked for all sessions.",
        type: "success",
      });
    } catch (error) {
      setToast({ message: errorMessage(error), type: "error" });
  ...
  return (
    <div className="space-y-8">
      ...
      <PageHeader
        title="B2B Access Password"
        subtitle="One shared password protects the read-only Gold trade catalogue."
      />
      <div className="admin-form max-w-2xl">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-[var(--color-text-muted)]">
              Last changed:{" "}
              {status.lastChanged
                ? new Date(status.lastChanged).toLocaleString()
                : "Never"}
            </p>
            <p className="mt-1 text-xs text-[var(--color-text-muted)]">
              Last verified mobile:{" "}
              {status.lastAccessMobile || "No verified access yet"}
            </p>
          </div>
        ...
        <form
          onSubmit={save}
          className="space-y-4 border-t border-[var(--color-border)] pt-5"
        >
          <label className="block">
            ...
            <div className="mt-2 flex gap-2">
              ...
              <button
                type="button"
                onClick={generate}
                className="px-4 border border-[var(--color-border)] rounded-[var(--radius-sm)] text-xs font-semibold cursor-pointer"
              >
                Generate
              </button>
            </div>
          ...
          <button className="admin-button cursor-pointer">
            Activate / replace password
          </button>
        </form>
        {status.active && (
          <button
            onClick={() => void revoke()}
            className="border border-[var(--color-error)]/40 text-[var(--color-error)] rounded-[var(--radius-sm)] px-4 py-2 text-xs font-semibold cursor-pointer"
          >
...
function B2BAccessLog() {
  const [entries, setEntries] = useState<
    import("../api/admin.api").B2BAccessLog[] | null
  >(null);
  ...
  if (error)
    return (
      <div className="space-y-8">
        <PageHeader
          title="B2B Access Log"
          subtitle="Successful B2B password and OTP verifications."
        />
        <p className="rounded-[var(--radius-sm)] border border-[var(--color-error)]/30 bg-[var(--color-error)]/10 p-3 text-sm text-[var(--color-error)]">
          {error}
        </p>
      </div>
    );
      ...
      <div className="overflow-hidden rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-secondary)]">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[560px] text-left">
            <thead className="border-b border-[var(--color-border)] bg-[var(--color-cream)]/40 text-xs uppercase tracking-wider text-[var(--color-text-muted)]">
              <tr>
                <th className="px-5 py-4 font-semibold">Mobile Number</th>
                <th className="px-5 py-4 font-semibold">Accessed At</th>
                <th className="px-5 py-4 font-semibold">OTP Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[var(--color-border)]">
              {entries.length ? (
                ...
                <tr>
                  ...
                  >
                    No successful B2B access has been recorded yet.
                  </td>
                </tr>
              )}
            </tbody>
...
// --- METAL RATES ---
function MetalRates() {
  const [rates, setRates] = useState<{ b2c: any; b2b: any } | null>(null);
  ...
  if (!rates)
    return <LoadingState title="Retrieving B2C and B2B metal rates..." />;
  const current = rates[tab],
    b2b = tab === "b2b";
  const save = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const f = new FormData(e.currentTarget);
    const body: Record<string, number | string> = { sheet: tab.toUpperCase() };
    if (!b2b) {
      body.gold24kt = Number(f.get("gold24kt"));
      body.silver = Number(f.get("silver"));
    }
    [
      "makingRatePerGram",
    ...
    ].forEach((k) => (body[k] = Number(f.get(k))));
    try {
      setRates(
        await apiRequest<any>("/admin/metal-rates", {
          method: "PUT",
          body: JSON.stringify(body),
        }),
      );
      setToast({
        message: `${tab.toUpperCase()} metal rates updated.`,
        type: "success",
      });
    } catch (error) {
      setToast({ message: errorMessage(error), type: "error" });
  ...
  const field = (key: string, label: string, locked = false) => (
    <label key={key} className="uppercase">
      {label}
      <input
        className="admin-input disabled:cursor-not-allowed disabled:bg-stone-100"
        name={key}
        type="number"
        min="0"
        step="any"
  ...
  return (
    <div className="space-y-8">
      ...
      <PageHeader
        title="Metal Rates"
        subtitle="Separate B2C and B2B rate sheets. Gold and Silver are shared from B2C."
      />
      <div className="flex gap-2 border-b border-[var(--color-border)]">
        <button
          type="button"
          onClick={() => setTab("b2c")}
          className={`px-4 py-2 text-sm font-semibold ${!b2b ? "border-b-2 border-[var(--color-teal)] text-[var(--color-teal)]" : "text-[var(--color-text-muted)]"}`}
        >
          B2C Metal Rates
        </button>
        <button
      ...
      <form key={tab} onSubmit={save} className="admin-form max-w-2xl">
        <b>Baseline Rate Configuration</b>
        {field("gold24kt", "Gold 24kt rate / 10g", b2b)}
        {field("silver", "Fine silver rate (per gram)", b2b)}
        {b2b && (
          <p className="col-span-full -mt-2 text-xs text-[var(--color-text-muted)]">
            Gold and Silver are synced from B2C - edit them in the B2C tab.
          </p>
        )}
        {field("makingRatePerGram", "Universal Gold making rate / gram")}
        {field("certificateRatePerGram", "Certificate charge / carat")}
        <div className="col-span-full border-t border-[var(--color-border)] pt-5">
          <b>Silver Category Making Rates</b>
        </div>
        {field(
          "silverMoissaniteMakingRate",
          "Silver-Moissanite making rate / gram",
        )}
        {field("silverPolkiMakingRate", "Silver-Polki making rate / gram")}
        {field("moissaniteRatePerCarat", "Moissanite rate per carat")}
        <div className="col-span-full flex items-center justify-between border-t border-[var(--color-border)] pt-4">
          <span className="text-xs text-[var(--color-text-muted)]">
            Last metal-rate update:{" "}
            {current.updatedAt
              ? new Date(current.updatedAt).toLocaleString()
              : "N/A"}
          </span>
          <button className="admin-button cursor-pointer">
            Save {tab.toUpperCase()} rates
          </button>
        </div>
      </form>
    </div>
...
function Categories() {
  ...
  const [categoryShortCode, setCategoryShortCode] = useState("");
  const [homepageCoverImage, setHomepageCoverImage] = useState("");
  ...
  const mainCategories = items.filter(
    (category) => category.categoryKind === "metal-root",
  );
  const parentCategories = items.filter(
    (category) =>
      category.categoryKind === "metal-root" ||
      category.categoryKind === "type",
  );
  const parentId = (category: Category) =>
  ...
  const resetForm = () => {
    ...
    setSelectedParentId("");
    setCategoryName("");
    setCategoryShortCode("");
    setHomepageCoverImage("");
  };
  const previewMetal = selectedParentId
    ? items.find((category) => category._id === selectedParentId)?.metal
    : categoryName.trim().toLowerCase() === "silver"
      ? "silver"
      : "gold";
  const fallbackShortCode = categoryName
    .replace(/[^a-z]/gi, "")
    .slice(0, 2)
    .toUpperCase()
    .padEnd(2, "X");
  const skuPreview = `TBA-${previewMetal === "silver" ? "SLV" : "GLD"}-${categoryShortCode || fallbackShortCode}-XXX`;
  const save = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const form = e.currentTarget;
    const f = new FormData(form);
    ...
    if (isUploadingCover) {
      setToast({
        message: "Wait for the cover image upload to finish",
        type: "error",
      });
      return;
    }
    if (isSubCategory && !parent) {
      setToast({ message: "Select a main category", type: "error" });
      return;
    }
    const payload = {
      ...
      showOnHomepage: f.get("showOnHomepage") === "on",
      homepageCoverImage: homepageCoverImage.trim(),
      shortCode: categoryShortCode,
    };
    if (payload.showOnHomepage && !payload.homepageCoverImage) {
      setToast({
        message: "Upload or enter a homepage cover image before saving",
        type: "error",
      });
      return;
    }
    const candidateMetal = parent
      ? items.find((category) => category._id === parent)?.metal
      : payload.name.toLowerCase();
    const duplicate = items.some(
      (category) =>
        category._id !== editing?._id &&
        category.name.trim().toLowerCase() === payload.name.toLowerCase() &&
        (parentId(category) || null) === parent &&
        category.metal === candidateMetal,
    );
    if (duplicate) {
      setToast({
        message:
  ...
  const startEdit = (category: Category) => {
    ...
    setIsSubCategory(Boolean(id));
    setSelectedParentId(id || "");
    setCategoryName(category.name);
    setCategoryShortCode(category.shortCode || "");
    setHomepageCoverImage(category.homepageCoverImage || "");
  };
  ...
  const toggleHomepage = async (category: Category) => {
    try {
      await adminApi.updateCategory(category._id, {
  ...
  const canDelete = (category: Category) =>
    category.categoryKind !== "metal-root" &&
    !items.some((item) => parentId(item) === category._id);
  const renderRow = (category: Category, nested = false) => (
    <div
      key={category._id}
      className={`admin-row justify-between ${nested ? "ml-6 border-l-2 border-[var(--color-border)] pl-4" : ""}`}
    >
      <div>
        ...
        <small>
          {nested ? "Sub Category" : "Main Category"} &bull;{" "}
          {category.homepageCoverImage
            ? "Homepage cover ready"
            : "No homepage cover"}{" "}
          | SKU code:{" "}
          {category.shortCode ||
            category.name
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => startEdit(category)} className="cursor-pointer">
          Edit
        </button>
        <button
  ...
  return (
    <div className="space-y-8">
      ...
      <PageHeader
        ...
        subtitle="Structure catalogue navigation and filters"
      />
      <form onSubmit={save} className="admin-form max-w-xl">
        ...
        <label>
          ...
          <select
            ...
            onChange={(e) => {
              const sub = e.target.value === "sub";
              setIsSubCategory(sub);
              if (!sub) setSelectedParentId("");
            }}
            className="admin-input"
          >
        ...
        <label>
          ...
          <select
            name="shortCode"
            value={categoryShortCode}
            onChange={(e) => setCategoryShortCode(e.target.value)}
            className="admin-input"
          >
            <option value="">Use category-name fallback</option>
            {CATEGORY_SHORT_CODES.map(([code, label]) => (
          ...
          <small>
            When blank, SKUs use the first two letters of the category name.
          </small>
        </label>
        <div className="col-span-full rounded border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-3 py-2 text-sm">
          <span className="text-[var(--color-text-muted)]">SKU preview: </span>
          <span className="font-mono font-semibold">{skuPreview}</span>
        </div>
        {isSubCategory && (
          <>
            <label>
              Homepage Cover Image Link
              <input
                value={homepageCoverImage}
                onChange={(e) => setHomepageCoverImage(e.target.value)}
                placeholder="Paste image URL"
                className="admin-input"
              />
            </label>
            <label>
              Upload Homepage Cover Image
              <input
                type="file"
                accept="image/*"
            ...
            {isUploadingCover && <small>Uploading cover image...</small>}
          </>
        )}
        <div className="col-span-full flex items-center justify-between">
          <div className="flex items-center gap-4">
            <label className="flex items-center gap-2 text-xs font-semibold text-[var(--color-text-muted)]">
              <input
                name="isActive"
                key={`${editing?._id || "new"}-active`}
                type="checkbox"
                defaultChecked={editing?.isActive ?? true}
                className="rounded border-[var(--color-border)] text-[var(--color-teal)]"
              />
              Active
            </label>
            {isSubCategory && (
              <label className="flex items-center gap-2 text-xs font-semibold text-[var(--color-text-muted)]">
                <input
                  name="showOnHomepage"
                  key={`${editing?._id || "new"}-homepage`}
                  type="checkbox"
                  defaultChecked={editing?.showOnHomepage ?? false}
                  className="rounded border-[var(--color-border)] text-[var(--color-teal)]"
                />
                Show on Homepage
              </label>
      ...
      {items.length === 0 ? (
        <EmptyState
          ...
          description="Create initial catalogue categories."
          icon={<Sparkles aria-hidden="true" />}
        />
      ) : (
        <div className="space-y-2">
          {mainCategories.map((main) => (
            <React.Fragment key={main._id}>
              {renderRow(main)}
              {items
                .filter((category) => parentId(category) === main._id)
                .map((sub) => (
                  <React.Fragment key={sub._id}>
                    {renderRow(sub, true)}
                    {items
                      .filter((category) => parentId(category) === sub._id)
                      .map((leaf) => renderRow(leaf, true))}
                  </React.Fragment>
                ))}
            </React.Fragment>
          ))}
...
function Orders() {
  ...
  const filtered = orders.filter((order: any) =>
    `${order._id || ""} ${order.customer?.name || ""} ${order.customer?.email || ""}`
      .toLowerCase()
  ...
  return (
    <div className="space-y-8">
      ...
      <PageHeader
        ...
        subtitle="A clear read-only record of every purchase."
      />
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <input
          value={search}
          onChange={(event) => setSearch(event.target.value)}
          placeholder="Search order reference, customer or email"
          className="admin-input w-full sm:max-w-md"
        />
        <span className="text-xs text-[var(--color-text-muted)]">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-5">
          {filtered.map((order: any) => (
            <article
              key={order._id}
              className="overflow-hidden rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-bg-secondary)] shadow-[var(--shadow-sm)]"
            >
              <header className="flex flex-col gap-4 border-b border-[var(--color-border)] bg-[var(--color-cream-light)]/60 p-5 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="mt-1 font-mono text-sm font-bold text-[var(--color-teal)]">
                    {reference(order._id)}
                  </p>
                </div>
                <div className="flex items-center gap-4 md:text-right">
                  <div>
                    ...
                    <p className="mt-1 text-lg font-bold text-[var(--color-teal)]">
                  ...
                  <button
                    ...
                    className="rounded border border-[var(--color-teal)] px-3 py-2 text-xs font-semibold text-[var(--color-teal)]"
                  >
                ...
                <div className="grid gap-5 p-5 lg:grid-cols-[minmax(0,1fr)_220px]">
                  <section>
                    <div className="mb-3 flex items-center justify-between gap-3">
                      ...
                      <span className="rounded-full bg-[var(--color-cream-light)] px-2.5 py-1 text-xs font-medium text-[var(--color-teal)]">
                    ...
                    <div className="space-y-2">
                      {(order.items || []).map((item: any, index: number) => (
                        <div
                          key={`${item.productSku || item.title}-${index}`}
                          className="flex gap-3 rounded-[var(--radius-sm)] border border-[var(--color-border)] bg-white p-3"
                        >
                          {item.image ? (
                            <img
                              src={item.image}
                            ...
                            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded bg-[var(--color-cream-light)] text-[10px] text-[var(--color-text-muted)]">
                              No image
                            </div>
                          )}
                          <div className="min-w-0 flex-1">
                            <p className="truncate font-semibold text-[var(--color-charcoal)]">
                              {item.title || item.productSku || "Product"}
                            </p>
                            <p className="mt-1 text-xs text-[var(--color-text-muted)]">
                              Selected Color: {item.color || "Not recorded"}
                            </p>
                            {item.size && (
                              <p className="mt-1 text-xs text-[var(--color-text-muted)]">
                            ...
                            <p className="mt-1 text-xs font-medium text-[var(--color-teal)]">
                              Quantity: {item.quantity || 1}
                            </p>
                          </div>
                          {item.priceSnapshot?.finalPrice != null && (
                  ...
                  <aside className="border-t border-[var(--color-border)] pt-4 lg:border-l lg:border-t-0 lg:pl-5 lg:pt-0">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--color-text-muted)]">
                      Customer
                    </p>
                    ...
                    <p className="mt-5 text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--color-text-muted)]">
                      Placed on
                    </p>
  ...
  return (
    <div className="space-y-8">
      ...
      <PageHeader
        ...
        subtitle="Manage accounts and customer access controls"
      />
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description="Customer account details will be listed here."
          icon={<Sparkles aria-hidden="true" />}
        />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map((u) => (
            <div key={u._id} className="admin-row justify-between">
              ...
              <button
                onClick={() =>
                  adminApi.setUserBlocked(u._id, !u.isBlocked).then(() => {
                    setToast({
                      message: `Customer ${!u.isBlocked ? "Blocked" : "Unblocked"}`,
                      type: "info",
                    });
                    void load();
                  })
