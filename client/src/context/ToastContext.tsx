...
interface ToastContextValue { toasts: Toast[]; showToast: (message: string, type?: ToastType) => void; removeToast: (id: string) => void; }
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const showToast = useCallback((message: string, type: ToastType = "info") => {
    setToasts((prev) => {
      ...
      const duration = type === "success" ? 4000 : type === "error" ? 8000 : 5000;
      return [...prev, { id: `${Date.now()}-${Math.random()}`, message, type, duration }].slice(-4);
    });
  }, []);
