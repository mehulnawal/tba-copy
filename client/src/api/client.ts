...
const repairMojibakeString = (value: string) => {
  ...
  for (
    let attempt = 0;
    attempt < 3 && mojibakeMarker.test(repaired);
    attempt += 1
  ) {
    const bytes = Uint8Array.from(
      Array.from(repaired, (character) => {
    ...
    try {
      ...
      if (
        !decoded ||
        decoded === repaired ||
        (mojibakeMarker.test(decoded) && decoded.length >= repaired.length)
      )
        break;
      repaired = decoded;
    } catch {
...
const requestOnce = async (endpoint: string, options: RequestInit = {}) => {
  ...
  return {
    response,
    data: repairMojibake(await response.json().catch(() => ({}))) as
...
const refreshAdminSession = async () => {
  if (!adminRefreshInFlight) {
    adminRefreshInFlight = requestOnce("/admin/auth/refresh", {
      method: "POST",
    })
