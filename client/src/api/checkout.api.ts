...
export interface CartSummary { subtotal: number; discount: number; referenceDiscount?: number; taxableSubtotal: number; gst: number; shippingFee: number; total: number; itemCount: number; }
...
export const checkoutApi = {
  ...
  applyReferenceId: (referenceId: string) => apiRequest<CheckoutSummaryResponse>("/checkout/reference-id", { method: "POST", body: JSON.stringify({ referenceId }) }),
  removeReferenceId: () => apiRequest<CheckoutSummaryResponse>("/checkout/reference-id", { method: "DELETE" }),
