...
export default function PrimeSelection() {
    ...
    const CompactProductCard = ({ product }: { product: Product }) => {
        ...
        return <article className="group relative flex gap-3 rounded-lg border border-[var(--color-border-subtle)] bg-white p-2.5 shadow-sm"><div className="relative h-32 w-28 shrink-0 overflow-hidden rounded-md bg-[var(--color-bg-secondary)] sm:h-40 sm:w-36"><img src={product.images[imageIndex]} alt={product.name} loading="lazy" decoding="async" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />{product.images.length > 1 && <><button type="button" onClick={() => setImageIndices((current) => ({ ...current, [product.id]: (imageIndex - 1 + product.images.length) % product.images.length }))} className="absolute left-1 top-1/2 -translate-y-1/2 rounded-full bg-white/90 p-1 text-[var(--color-teal)] shadow" aria-label="Previous product image"><ChevronLeft className="h-3 w-3" /></button><button type="button" onClick={() => setImageIndices((current) => ({ ...current, [product.id]: (imageIndex + 1) % product.images.length }))} className="absolute right-1 top-1/2 -translate-y-1/2 rounded-full bg-white/90 p-1 text-[var(--color-teal)] shadow" aria-label="Next product image"><ChevronRight className="h-3 w-3" /></button></>}</div><div className="min-w-0 flex flex-1 flex-col py-0.5"><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><p className="font-mono text-[9px] uppercase tracking-wider text-[var(--color-text-muted)]">{product.code}</p><h4 className="mt-1 font-primary text-base leading-snug text-[var(--color-text)]">{product.name}</h4><p className="mt-1 text-[10px] tracking-wide text-[var(--color-text-muted)]">{product.category}</p></div><button type="button" onClick={() => setWishlistedProducts((current) => ({ ...current, [product.id]: !isWishlisted }))} aria-label={isWishlisted ? "Remove from wishlist" : "Add to wishlist"} className="rounded-full border border-[var(--color-border-subtle)] bg-white p-1.5 text-[var(--color-text-muted)] hover:text-rose-600"><Heart className={`h-3.5 w-3.5 ${isWishlisted ? "fill-rose-600 stroke-rose-600" : ""}`} /></button></div><div className="mt-auto border-t border-[var(--color-border-subtle)] pt-3"><div className="flex items-end justify-between gap-2"><div><p className="text-[8px] uppercase tracking-widest text-[var(--color-text-muted)]">Estimated Price</p><p className="text-xs font-semibold text-[var(--color-text)]">{"?"}{selectedPrice.toLocaleString("en-IN")}</p></div><div className="flex gap-1">{product.prices.map((price) => <button key={price.karat} type="button" onClick={() => setSelectedKarats((current) => ({ ...current, [product.id]: price.karat }))} className={`rounded border px-1.5 py-0.5 text-[8px] font-semibold ${selectedKarat === price.karat ? "border-[var(--color-teal)] bg-[var(--color-cream-light)] text-[var(--color-teal)]" : "border-[var(--color-border-subtle)] text-[var(--color-text-muted)]"}`}>{price.karat.toUpperCase()}</button>)}</div></div><button type="button" className="mt-3 flex w-full items-center justify-center gap-1.5 rounded bg-[var(--color-teal)] px-3 py-2.5 text-[9px] font-medium uppercase tracking-widest text-white transition-colors hover:bg-[var(--color-teal-light)]"><ShoppingBag className="h-3 w-3" />Add to Cart</button></div></div></article>;
    };
    return (
                            ...
                            <AnimatePresence>
                                {showDrawer && (
                                    <motion.div
                                        ...
                                        className="bg-white w-full min-h-[390px] max-h-[92%] rounded-xl p-8 shadow-2xl border border-zinc-100/80 pointer-events-auto flex flex-col z-50"
                                    >
                    ...
                    <AnimatePresence>
                        <div className="fixed inset-0 z-[999999] flex items-end justify-center">
                            ...
                            <motion.div
                                ...
                                className="relative bg-white w-full min-h-[52dvh] max-h-[82dvh] rounded-t-2xl px-6 pb-7 pt-5 shadow-[0_-8px_32px_rgba(0,0,0,0.15)] z-50 overflow-hidden flex flex-col"
                            >
