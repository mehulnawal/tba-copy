...
export default function PriceBreakup({ product, price, coupon, b2b = false, className = "" }: Props) {
  ...
  const diamondEntries = (product.diamonds || []).map((entry, index) => ({
    key: `diamond-${index}`,
  ...
  const totals = (fixedColumns: boolean, columnCount = 2) => <div>
    <div className="overflow-x-auto">
      <table className={`w-full text-left text-xs${fixedColumns ? " table-fixed" : ""}`}>
        ...
        <tbody>
          <tr className="border-b border-[var(--color-border)]"><td className="py-2" colSpan={fixedColumns ? columnCount - 1 : 1}><h3 className="font-bold [-webkit-text-stroke:0.2px_currentColor] text-[14px] text-[var(--color-teal)]">{coupon ? "Original Subtotal" : "Subtotal"}</h3></td><td className={fixedColumns ? "whitespace-nowrap py-2 text-right tabular-nums" : "py-2 text-right"}>{formatINR(number(coupon ? coupon.subtotalAfterCoupon + coupon.discount : price.totalCost))}</td></tr>
          {coupon && <><tr className="border-b border-[var(--color-border)]"><td className="py-2" colSpan={fixedColumns ? columnCount - 1 : 1}>Coupon Applied - {coupon.label}</td><td className={fixedColumns ? "whitespace-nowrap py-2 text-right tabular-nums text-[var(--color-teal)]" : "py-2 text-right text-[var(--color-teal)]"}>-{formatINR(number(coupon.discount))}</td></tr><tr className="border-b border-[var(--color-border)]"><td className="py-2" colSpan={fixedColumns ? columnCount - 1 : 1}><h3 className="font-bold [-webkit-text-stroke:0.2px_currentColor] text-[14px] text-[var(--color-teal)]">Subtotal After Coupon</h3></td><td className={fixedColumns ? "whitespace-nowrap py-2 text-right tabular-nums" : "py-2 text-right"}>{formatINR(number(coupon.subtotalAfterCoupon))}</td></tr></>}
    ...
    {hasMoissanite && <div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full table-fixed text-left text-xs">
          ...
          <tbody><tr className="border-b border-[var(--color-border)]"><td className="py-2">Moissanite</td><td className="whitespace-nowrap py-2 text-right tabular-nums">{formatINR(number(price.moissaniteRatePerCarat))}</td><td className="whitespace-nowrap py-2 text-right tabular-nums">{formatMeasurement(number(price.totalMoissaniteWeight ?? product.moissaniteCaratWeight))} ct</td><td className="whitespace-nowrap py-2 text-right tabular-nums">{formatINR(number(price.moissaniteValue))}</td></tr></tbody>
        </table>
      </div>
    </div>}
    {hasMoissanite && diamondEntries.length > 0 && <div>
      <h3 className="py-2 font-bold [-webkit-text-stroke:0.2px_currentColor] text-[var(--color-teal)]">Diamond Charges</h3>
      <div className="overflow-x-auto">
        <table className="w-full min-w-[340px] table-fixed text-left text-xs">
          <colgroup><col className="w-[28%]" /><col className="w-[18%]" /><col className="w-[18%]" /><col className="w-[18%]" /><col className="w-[18%]" /></colgroup>
          <thead className="border-t border-[var(--color-border)] text-[var(--color-text-muted)]"><tr><th className="py-2 font-bold [-webkit-text-stroke:0.2px_currentColor]">Component</th><th className="py-2 font-bold [-webkit-text-stroke:0.2px_currentColor]">Colour/Clarity</th><th className="py-2 text-right font-bold [-webkit-text-stroke:0.2px_currentColor]">Ct</th><th className="py-2 text-right font-bold [-webkit-text-stroke:0.2px_currentColor]">Rate/Ct</th><th className="py-2 text-right font-bold [-webkit-text-stroke:0.2px_currentColor]">Price</th></tr></thead>
          <tbody>{diamondEntries.map(entry => <tr key={entry.key} className="border-b border-[var(--color-border)]"><td className="py-2">{entry.component}</td><td className="py-2">{entry.clarity}</td><td className="whitespace-nowrap py-2 text-right tabular-nums">{formatMeasurement(entry.carat)}</td><td className="whitespace-nowrap py-2 text-right tabular-nums">{formatINR(entry.rate)}</td><td className="whitespace-nowrap py-2 text-right tabular-nums">{formatINR(entry.value)}</td></tr>)}</tbody>
        </table>
      </div>
    </div>}
    {totals(true, 3)}
  ...
  const goldBreakup = <>
    ...
    {stoneEntries.length > 0 && <div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[340px] table-fixed text-left text-xs sm:min-w-[16px]">
          <colgroup><col className="w-[23%]" /><col className="w-[21%]" /><col className="w-[9%]" /><col className="w-[24%]" /><col className="w-[23%]" /></colgroup><thead className="border-t border-[var(--color-border)] text-[var(--color-text-muted)]"><tr>
            <th className="py-2 font-bold [-webkit-text-stroke:0.2px_currentColor]">Component</th>
            <th className="py-2 font-bold [-webkit-text-stroke:0.2px_currentColor]">
              <span className="block sm:inline">Colour<span className="hidden sm:inline">/</span>
              </span>
              <span className="block sm:inline">Clarity</span>
            </th>
            <th className="py-2 font-bold [-webkit-text-stroke:0.2px_currentColor]">Ct</th>
            <th className="py-2 text-right font-bold [-webkit-text-stroke:0.2px_currentColor]">Rate/Ct</th>
            <th className="py-2 text-right font-bold [-webkit-text-stroke:0.2px_currentColor]">Price</th>
          </tr>
          </thead>
          <tbody>{stoneEntries.map(entry => <tr key={entry.key} className="border-b border-[var(--color-border)]">
            <td className="py-2">{entry.component}</td><td className="py-2">{entry.clarity}</td>
            <td className="py-2">{formatMeasurement(entry.carat)}</td><td className="whitespace-nowrap py-2 text-right tabular-nums">{formatINR(entry.rate)}</td>
            <td className="whitespace-nowrap py-2 text-right tabular-nums">{formatINR(entry.value)}</td>
          </tr>)}</tbody>
        </table>
      </div>
    </div>}
