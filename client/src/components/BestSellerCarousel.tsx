      ...
      <div className="overflow-hidden" ref={emblaRef}>
        <div className="flex gap-x-6 touch-pan-y">
          {filteredProducts.map((product) => {
            ...
            const priceOptions = Array.isArray(product.prices)
              ? Array.from(
                  new Map(
                    product.prices
                      .filter(
                        (price: { karat?: string }) =>
                          price.karat === "14kt" || price.karat === "18kt",
                      )
                      .map((price: { karat: string }) => [price.karat, price]),
                  ).values(),
                )
              : [];
            const selectedKarat =
              selectedKarats[product.id] ||
              priceOptions[0]?.karat ||
              product.karat;
            const selectedPrice =
              priceOptions.find(
                (price: { karat?: string }) => price.karat === selectedKarat,
              ...
              >
                ...
                >
                      ...
                      {images.length > 1 && (
                        <>
                          <button
                            ...
                            className="absolute left-2 top-1/2 z-20 -translate-y-1/2 rounded-full bg-white/90 p-1.5 text-[var(--color-teal)] shadow"
                          ...
                          <button
                            ...
                            onClick={(event) => {
                              ...
                              setImageIndices((current) => ({
                                ...current,
                                [product.id]: (imageIndex + 1) % images.length,
                              }));
                            }}
                            className="absolute right-2 top-1/2 z-20 -translate-y-1/2 rounded-full bg-white/90 p-1.5 text-[var(--color-teal)] shadow"
                  ...
                  <button
                    ...
                    aria-label={
                      isInWishlist ? "Remove from vault" : "Add to vault"
                    }
                  ...
                  >
                    <Heart
                      className={`w-4 h-4 transition-transform duration-300 ${isInWishlist ? "fill-rose-600 stroke-rose-600" : "stroke-[1.5]"}`}
                    />
                  </button>
                  ...
                  <div className="flex flex-col gap-3 mt-auto pt-3 border-t border-[var(--color-border-subtle)] px-1 z-20">
                    ...
                    {priceOptions.length > 1 && (
                      <div className="flex gap-1">
                        {priceOptions.map((price: { karat: string }) => (
                          <button
                            key={price.karat}
                            type="button"
                            onClick={() =>
                              setSelectedKarats((current) => ({
                                ...current,
                                [product.id]: price.karat,
                              }))
                            }
                            className={`rounded border px-1.5 py-0.5 text-[9px] font-semibold ${selectedKarat === price.karat ? "border-[var(--color-teal)] bg-[var(--color-cream-light)] text-[var(--color-teal)]" : "border-[var(--color-border-subtle)] text-[var(--color-text-muted)]"}`}
                          >
      ...
      <div className="flex justify-center gap-2 mt-8">
        {scrollSnaps.map((_, index) => (
          <button
            ...
            className={`transition-all duration-300 rounded-full border-none cursor-pointer focus:outline-none ${
              index === selectedIndex
