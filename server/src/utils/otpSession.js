...
const resendOtpSession = async (value, requestId) => {
  const mobile = indianMobile(value);
  if (!validMobile(mobile) || !requestId)
    throw new ApiError(400, "A valid OTP request is required");
  const attempt = attemptFor(requestId, mobile);
  if (attempt.resends >= OTP_RESEND_LIMIT)
    throw new ApiError(429, "OTP resend limit reached");
  const retryAfter = attempt.resendAvailableAt - Date.now();
  if (retryAfter > 0)
    throw new ApiError(
      429,
      `Please wait ${Math.ceil(retryAfter / 1000)} seconds before resending OTP`,
    );
  ...
  return {
    ...
    resendCount: nextAttempt.resends,
    resendAvailableAt: nextAttempt.resendAvailableAt,
  };
};
...
const verifyOtpSession = async (value, otp, requestId) => {
  ...
  if (!/^\d{6}$/.test(String(otp || "")) || !requestId)
    throw new ApiError(400, "Enter the six-digit OTP");
