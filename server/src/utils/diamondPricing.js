...
const validPrice = (value, label) => {
  if (value === undefined || value === null || !Number.isFinite(Number(value)))
    throw new Error(`${label} is required for diamond pricing`);
  return Number(value);
};
 ...
 */
const getDiamondPricing = async ({
  productId,
  diamondCategoryRef,
  product,
} = {}) => {
  const resolvedProduct =
    product || (productId ? await Product.findById(productId).lean() : null);
  if (productId && !resolvedProduct) throw new Error("Product not found");
  const override = resolvedProduct?.diamondPriceOverride;
  if (override)
    return {
      b2bPrice: validPrice(override.b2bPrice, "Diamond B2B price"),
      b2cPrice: validPrice(override.b2cPrice, "Diamond B2C price"),
    };

  const categoryRef = resolvedProduct?.diamondCategoryRef || diamondCategoryRef;
  if (!categoryRef)
    throw new Error("Diamond category reference is required for pricing");
  const category = await DiamondCategory.findById(categoryRef).lean();
  if (!category) throw new Error("Diamond category not found");
  return {
    b2bPrice: validPrice(category.b2bPrice, "Diamond category B2B price"),
    b2cPrice: validPrice(category.b2cPrice, "Diamond category B2C price"),
  };
};
...
const hydrateLiveDiamondEntryRates = async (product) => {
  ...
  const diamonds = Array.isArray(rawProduct?.diamonds)
    ? rawProduct.diamonds
    : [];
  const productCategoryRef =
    rawProduct?.metal === "gold" ? rawProduct.diamondCategoryRef : null;
  const ids = [
    ...new Set(
      [
        ...diamonds.map((entry) => entry?.diamondCategoryRef),
        productCategoryRef,
      ]
        .filter(Boolean)
        .map(String),
  ...
  const categories = await DiamondCategory.find({
    $or: [{ _id: { $in: ids } }, { categoryName: { $in: names } }],
  })
    .select("categoryName size b2bPrice b2cPrice")
    .lean();
  const byId = new Map(
    categories.map((category) => [String(category._id), category]),
  );
  const productCategory = productCategoryRef
    ? byId.get(String(productCategoryRef))
    : null;
  const byName = new Map(
    categories.reduce((groups, category) => {
      const key = String(category.categoryName || "")
        .trim()
        .toLowerCase();
  ...
  return {
    ...rawProduct,
    diamonds: diamonds.map((entry, index) => {
      const candidates =
        byName.get(
        ...
        : candidates.find(
            (item) =>
              ...
              String(entry?.subType || "")
                .trim()
                .toLowerCase(),
          ) || (candidates.length === 1 ? candidates[0] : productCategory);
      if (entry?.diamondCategoryRef && !category)
        throw new Error(`Diamond ${index + 1} category not found`);
      if (!category) return entry;
      return {
        ...entry,
        diamondCategoryRef: entry.diamondCategoryRef || category._id,
        ratePerCtB2B: validPrice(
          category.b2bPrice,
          `Diamond ${index + 1} B2B rate`,
        ),
        ratePerCtB2C: validPrice(
          category.b2cPrice,
          `Diamond ${index + 1} B2C rate`,
        ),
      };
    }),
  };
