...
const calculateSilverPrice = ({ product, rates, config }) => {
  ...
  const metalValue = grossWeight * silverRate;
  const makingValue = grossWeight * makingRate;
  const isMoissanite = String(config.categoryType).toLowerCase().includes("moissanite");
  const stoneValue = isMoissanite ? requiredNumber(product?.moissaniteCaratWeight, "Moissanite carat weight") * requiredNumber(config.stoneRatePerUnit, "Moissanite rate per ct") : 0;
  const totalCost = metalValue + makingValue + stoneValue;
  const gst = totalCost * GST_RATE;
  return { metal: "silver", silverRate, grossWeight, metalValue, makingRatePerGram: makingRate, makingValue, stoneValue, totalCost, gst, finalPrice: totalCost + gst };
};
