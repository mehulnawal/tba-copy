...
const createProduct = asyncHandler(async (req, res) => { const body = normalizeGoldWeights(req.body); await validateCategories(body); const category = await Category.findById(body.subCategory).select("name shortCode").lean(); const metalCode = body.metal === "gold" ? "GLD" : "SLV"; const categoryCode = category?.shortCode || String(category?.name || "XX").replace(/[^a-z]/gi, "").slice(0, 2).toUpperCase().padEnd(2, "X"); const prefix = `TBA-${metalCode}-${categoryCode}-`; const latest = await Product.findOne({ SKU: new RegExp(`^${prefix}\\d{3}$`) }).sort({ SKU: -1 }).select("SKU").lean(); const number = latest ? Number(String(latest.SKU).slice(-3)) + 1 : 1; const SKU = body.SKU && !String(body.SKU).startsWith("TBA-") ? body.SKU : `${prefix}${String(number).padStart(3, "0")}`; const product = await Product.create({ ...body, SKU, slug: slugify(body.title) }); res.status(201).json(new ApiResponse(201, await toProductResponse(await populated(Product.findById(product._id))), "Product created")); });
