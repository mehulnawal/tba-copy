...
const list = asyncHandler(async (req, res) =>
  res.json(
    new ApiResponse(
      200,
      await Certificate.find({ isActive: true }).sort({ name: 1 }).lean(),
...
const save = asyncHandler(async (req, res) => {
  ...
  res.status(201).json(new ApiResponse(201, item, "Certificate saved"));
});
const remove = asyncHandler(async (req, res) => {
  const item = await Certificate.findByIdAndDelete(req.params.certificateId);
  if (!item) throw new ApiError(404, "Certificate not found");
  res.json(new ApiResponse(200, null, "Certificate deleted"));
});
