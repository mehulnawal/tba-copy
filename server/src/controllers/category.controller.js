...
const deriveHierarchy = async ({ name, parent, categoryId }) => {
  if (!parent) {
    if (!["Gold", "Silver"].includes(name))
      throw new ApiError(
        400,
        "Only the fixed Gold and Silver roots may have no parent",
      );
    return {
  ...
  if (!parentCategory)
    throw new ApiError(400, "Parent category does not exist");
  if (parentCategory.categoryKind === "metal-root")
    return {
      metal: parentCategory.metal,
      categoryKind: parentCategory.metal === "gold" ? "subcategory" : "type",
      parent: parentCategory._id,
    };
  if (
    parentCategory.categoryKind === "type" &&
    parentCategory.metal === "silver"
  )
    return {
      metal: "silver",
  ...
  throw new ApiError(
    400,
    "Only Gold, Silver, or a Silver category type can contain categories",
  );
};
const categoryFilter = (req) => {
  const filter = { isActive: true };
  if (req.query.metal) {
    if (!["gold", "silver"].includes(req.query.metal))
...
const listPublic = asyncHandler(async (req, res) =>
  res.json(
    new ApiResponse(
      200,
      await withParent(Category.find(categoryFilter(req))),
      "Categories fetched",
    ),
  ),
);
const listAdmin = asyncHandler(async (req, res) => {
  ...
  res.json(
    new ApiResponse(
      200,
      await withParent(Category.find(filter)),
      "Categories fetched",
    ),
  );
});
const create = asyncHandler(async (req, res) => {
  ...
  const hierarchy = await deriveHierarchy({ name, parent: req.body.parent });
...
const update = asyncHandler(async (req, res) => {
  ...
  if (!name) throw new ApiError(400, "Category name is required");
  const parent = Object.prototype.hasOwnProperty.call(req.body, "parent")
    ? req.body.parent
    : existing.parent;
  const hierarchy = await deriveHierarchy({
    name,
    parent,
...
const remove = asyncHandler(async (req, res) => {
  ...
  if (category.categoryKind === "metal-root")
    throw new ApiError(400, "Gold and Silver roots cannot be deleted");
  if (await Category.exists({ parent: category._id }))
    throw new ApiError(400, "Delete or reassign child categories first");
  if (
    await Product.exists({
      $or: [{ mainCategory: category._id }, { subCategory: category._id }],
    })
  )
    throw new ApiError(
      400,
...
const initializeCategoryStructure = async () => {
  ...
  for (const [index, name] of FIXED_SUBCATEGORIES.entries())
    await Category.findOneAndUpdate(
      { name, parent: gold._id, metal: "gold" },
      {
        $setOnInsert: {
          name,
          metal: "gold",
  ...
  for (const [typeIndex, typeName] of ["Moissanite", "Polki"].entries()) {
    const type = await Category.findOneAndUpdate(
      { name: typeName, parent: silver._id, metal: "silver" },
      {
        $setOnInsert: {
          name: typeName,
          metal: "silver",
    ...
    for (const [index, name] of FIXED_SUBCATEGORIES.entries())
      await Category.findOneAndUpdate(
        { name, parent: type._id, metal: "silver" },
        {
          $setOnInsert: {
            name,
            metal: "silver",
