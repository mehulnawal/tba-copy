...
const validGoogleDriveFolderUrl = (value) => {
  try {
    ...
    return (
      url.protocol === "https:" &&
      /^(www\.)?drive\.google\.com$/i.test(url.hostname) &&
...
const folderCreated = asyncHandler(async (req, res) => {
  if (
    !secretsMatch(
      req.get("X-CAD-Webhook-Secret"),
      process.env.CAD_WEBHOOK_SECRET,
    )
  )
    throw new ApiError(401, "Unauthorized CAD webhook request");
  const sku = typeof req.body?.sku === "string" ? req.body.sku.trim() : "";
  const cadFolderUrl =
    typeof req.body?.cadFolderUrl === "string"
      ? req.body.cadFolderUrl.trim()
      : "";
  if (!sku) throw new ApiError(400, "SKU is required");
  if (!validGoogleDriveFolderUrl(cadFolderUrl))
    throw new ApiError(
      400,
      "A valid HTTPS Google Drive folder URL is required",
    );
  const product = await Product.findOneAndUpdate(
    { SKU: sku },
    { $set: { cadFolderUrl } },
    { new: true, runValidators: true },
  ).select("SKU cadFolderUrl");
  ...
  res
    ...
    .json(
      new ApiResponse(
        200,
        { sku: product.SKU, cadFolderUrl: product.cadFolderUrl },
        "CAD folder saved",
      ),
    );
});
