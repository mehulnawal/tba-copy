...
const list = asyncHandler(async (req, res) => {
  const rows = await DiamondCategory.find({})
    .sort({ categoryName: 1 })
...
const get = asyncHandler(async (req, res) => {
  const row = await DiamondCategory.findById(
    req.params.diamondCategoryId,
  ).lean();
...
const create = asyncHandler(async (req, res) => {
  ...
  if (!categoryName)
    throw new ApiError(400, "Diamond category name is required");
  const row = await DiamondCategory.create({
    categoryName,
    size,
    b2bPrice,
    b2cPrice,
...
const update = asyncHandler(async (req, res) => {
  ...
  const categoryName = String(
    req.body.categoryName ?? current.categoryName,
  ).trim();
  ...
  if (!categoryName || !size)
    throw new ApiError(400, "Diamond category name and size are required");
  if (
    !Number.isFinite(b2bPrice) ||
    b2bPrice < 0 ||
    !Number.isFinite(b2cPrice) ||
    b2cPrice < 0
  )
    throw new ApiError(400, "B2B and B2C prices must be non-negative numbers");
    ...
    await Product.updateMany(
      {
        diamonds: {
          $elemMatch: {
            $or: [
              { diamondCategoryRef: current._id },
              { category: previousCategoryName, subType: previousSize },
            ],
          },
        },
      },
      {
        $set: {
          "diamonds.$[diamond].category": categoryName,
          "diamonds.$[diamond].subType": size,
        },
      },
      {
        arrayFilters: [
          {
            $or: [
              { "diamond.diamondCategoryRef": current._id },
              {
                "diamond.category": previousCategoryName,
                "diamond.subType": previousSize,
              },
            ],
          },
        ],
...
const remove = asyncHandler(async (req, res) => {
  ...
  const referenced = await Product.exists({
    diamonds: {
      $elemMatch: {
        $or: [
          { category: row.categoryName, subType: row.size },
          { categoryName: row.categoryName, size: row.size },
        ],
      },
    },
  });
  if (referenced)
    throw new ApiError(
      409,
      "Diamond category is in use by an existing product",
    );
  await row.deleteOne();
