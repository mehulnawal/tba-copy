...
const secret = () =>
  process.env.JWT_B2B_ACCESS_SECRET || process.env.JWT_ACCESS_SECRET;
const requireB2BAccess = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    throw new ApiError(401, "B2B session is invalid or expired");
  }
  const access = await B2BAccess.findOne({ key: "current" }).select(
    "+passwordHash",
  );
  if (!access?.isActive || decoded.version !== access.sessionVersion)
    throw new ApiError(401, "B2B access has been revoked");
